...
tchuekam kanban watch --tenant <project-slug>

# Snapshot of the board
tchuekam kanban list --tenant <project-slug>
tchuekam kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
tchuekam kanban stats --tenant <project-slug>

# Visual dashboard (browser)
tchuekam dashboard

# Inspect a specific task (includes comments + events)
tchuekam kanban show <task-id>

# Follow a single task's event stream
tchuekam kanban tail <task-id>
```

Verify available subcommands with `tchuekam kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `tchuekam kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `tchuekam kanban block <id>` then `tchuekam kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `tchuekam kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `tchuekam kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `tchuekam profile list` (profile exists?), `tchuekam status` (gateway/dispatcher up?), and verify tenant. |
...
tchuekam kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
tchuekam kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(tchuekam kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
tchuekam kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
tchuekam kanban block <task-id>
tchuekam kanban archive <task-id>

# Diagnose what happened
tchuekam kanban show <task-id>      # task body, comments, recent events
tchuekam kanban tail <task-id>      # follow the live event stream
tchuekam kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `tchuekam kanban create` for the director
...
while true; do
    clear
    tchuekam kanban list --tenant <slug>
    echo "---"
    tchuekam kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `tchuekam kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `tchuekam profile list` and start
