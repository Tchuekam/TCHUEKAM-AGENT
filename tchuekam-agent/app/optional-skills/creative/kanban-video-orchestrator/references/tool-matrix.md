...
Maps each role archetype to the TchuEkaM skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public tchuekam-agent
...
## TchuEkaM skills relevant to video production

### Visual / rendering skills (`tchuekam-agent/skills/creative/`)
...
### Design / pre-production skills (`tchuekam-agent/skills/creative/`)
...
### Audio / media skills (`tchuekam-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`tchuekam-agent/skills/devops/`)
...
These are **not** TchuEkaM skills but external CLIs / APIs that profiles invoke.
...
## Built-in TchuEkaM tools for media review

These are native TchuEkaM tools — not invoked via terminal but through their own
...
key is present in `~/.tchuekam/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every TchuEkaM profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.tchuekam/.env` → environment variable.
...
When a new TchuEkaM-public video skill ships:
