...
Usage (via cron with --no-agent):
    ...
    tchuekam cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$TCHUEKAM_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "TchuEkaM-Watcher/1.0"})
