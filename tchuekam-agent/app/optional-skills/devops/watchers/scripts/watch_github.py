...
Usage (via cron with --no-agent):
    ...
    tchuekam cron create tchuekam-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$TCHUEKAM_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name tchuekam-issues --repo NousResearch/tchuekam-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.tchuekam/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "TchuEkaM-Watcher/1.0",
    }
