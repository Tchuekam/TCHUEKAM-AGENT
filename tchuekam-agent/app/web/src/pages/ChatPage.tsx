/**
 * ChatPage — embeds `tchuekam --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (tchuekam_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { TCHUEKAM_BASE_PATH } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${TCHUEKAM_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__TCHUEKAM_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `tchuekam dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const token = window.__TCHUEKAM_SESSION_TOKEN__;
    ...
    const term = new Terminal({
      ...
      // even when the inner TchuEkaM TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[tchuekam-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into TchuEkaM. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="tchuekam-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __TCHUEKAM_SESSION_TOKEN__?: string;
  }
}
