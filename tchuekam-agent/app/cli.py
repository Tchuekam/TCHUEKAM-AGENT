#!/usr/bin/env python3
"""
TchuEkaM Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the TchuEkaM Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills tchuekam-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: tchuekam_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See tchuekam_bootstrap.py for full rationale.
try:
    import tchuekam_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when tchuekam_bootstrap isn't registered in the venv
    # yet — happens during partial ``tchuekam update`` where git-reset landed
...
os.environ["TCHUEKAM_QUIET"] = "1"  # Our own modules

import yaml

from tchuekam_cli.fallback_config import get_fallback_chain
...
try:
    from tchuekam_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from tchuekam_cli.banner import _format_context_length, format_banner_version_label
...
from tchuekam_cli.browser_connect import (
...
_tchuekam_home = get_tchuekam_home()
_project_env = Path(__file__).parent / '.env'
load_tchuekam_dotenv(tchuekam_home=_tchuekam_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.tchuekam/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from tchuekam_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.tchuekam/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If TCHUEKAM_IGNORE_USER_CONFIG=1 is set (via ``tchuekam chat --ignore-user-config``),
    the user config at ``~/.tchuekam/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({TCHUEKAM_HOME}/config.yaml)
    user_config_path = _tchuekam_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("TCHUEKAM_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain TchuEkaM, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me TchuEkaM - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see tchuekam_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from tchuekam_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and TchuEkaMCLI.__init__ checks "default" first.
    ...
    from tchuekam_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (tchuekam_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && tchuekam` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _TCHUEKAM_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_TCHUEKAM_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["TCHUEKAM_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.tchuekam/logs/.
...
try:
    from tchuekam_cli.config import print_config_warnings
...
try:
    from tchuekam_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``tchuekam --help`` paths inside the chat command flow).  See
...
from tchuekam_cli.banner import build_welcome_banner
from tchuekam_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from tchuekam_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("TCHUEKAM_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("TCHUEKAM_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _run_cleanup():
    ...
    try:
        from tchuekam_cli.plugins import invoke_hook as _invoke_hook
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run tchuekam -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"tchuekam-{short_id}"
    branch_name = f"tchuekam/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`tchuekam_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from tchuekam_cli.config import load_config as _load_full_config
        from tchuekam_constants import get_tchuekam_home as _get_tchuekam_home
        _tchuekam_home_maint = _get_tchuekam_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_tchuekam_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`tchuekam_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``tchuekam/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("tchuekam-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``tchuekam/tchuekam-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``tchuekam -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("tchuekam/tchuekam-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. TCHUEKAM_LIGHT / TCHUEKAM_TUI_LIGHT env (true/false) — explicit override
#   2. TCHUEKAM_TUI_THEME=light|dark — explicit theme
#   3. TCHUEKAM_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy TchuEkaM assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("TCHUEKAM_LIGHT", "TCHUEKAM_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("TCHUEKAM_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("TCHUEKAM_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from tchuekam_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_tchuekam_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._tchuekam_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from tchuekam_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in TchuEkaM output it's common to display cron schedules verbatim.
    #
...
from tchuekam_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``TchuEkaMCLI._busy_command()``.
        """
        yield self

# ASCII Art - TCHUEKAM-AGENT logo (full width, single line - requires ~95 char terminal)
TCHUEKAM_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - TchuEkaM Caduceus (compact, fits in left panel)
TCHUEKAM_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    border_color = _skin.get_color("banner_border", "#8f00ff") if _skin else "#8f00ff"
    title_color = _skin.get_color("banner_title", "#b026ff") if _skin else "#b026ff"
    dim_color = _skin.get_color("banner_dim", "#8f00ff") if _skin else "#8f00ff"

    if skin_name == "default":
        line1 = "◆ TCHUEKAM-AGENT :: Autonomous Enterprise Intelligence"
        tiny_line = "◆ TCHUEKAM-AGENT"
    else:
        agent_name = _skin.get_branding("agent_name", "TCHUEKAM Agent") if _skin else "TCHUEKAM Agent"
        line1 = f"{agent_name} - Autonomous Intelligence Platform"
        tiny_line = agent_name

    if os.environ.get("TCHUEKAM_FAST_STARTUP_BANNER") == "1":
        from tchuekam_cli import __release_date__ as _release_date
        from tchuekam_cli import __version__ as _version

        version_line = f"TCHUEKAM-AGENT v1.0.0 (ENTERPRISE)"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- TCHUEKAM SYSTEMS[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from tchuekam_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.tchuekam/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.tchuekam/config.yaml on first use)
...
class TchuEkaMCLI:
    """
    Interactive CLI for the TchuEkaM Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from tchuekam_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("TCHUEKAM_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("TCHUEKAM_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("TCHUEKAM_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `tchuekam chat --ignore-rules` in tchuekam_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("TCHUEKAM_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("TCHUEKAM_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all TchuEkaM processes for this TCHUEKAM_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.tchuekam/checkpoints/.  Opt-in via
        ...
        self._history_file = _tchuekam_home / ".tchuekam_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from tchuekam_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("TCHUEKAM_YOLO_MODE"))
            if width < 52:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'TchuEkaM'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from tchuekam_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from tchuekam_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from tchuekam_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from tchuekam_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "◆ TCHUEKAM")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - TchuEkaMCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from tchuekam_cli.runtime_provider import (
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: tchuekam setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: tchuekam setup")
            return False
        ...
        # `tchuekam chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `tchuekam auth add openai-codex`
        # without `tchuekam model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from tchuekam_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from tchuekam_cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (tchuekam sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``tchuekam doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``tchuekam doctor`` so the banner stays
        small.
        """
        try:
            from tchuekam_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   TchuEkaM needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Nous TchuEkaM LLM (not agentic)
        from tchuekam_cli.model_switch import is_nous_tchuekam_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_tchuekam_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research TchuEkaM 3 & 4 models are NOT agentic and are not "
                "designed for use with TchuEkaM Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ TCHUEKAM: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ TCHUEKAM: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.tchuekam/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from tchuekam_cli.clipboard import save_clipboard_image

        img_dir = get_tchuekam_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: tchuekam --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for TchuEkaM config/state.
        ...
        """
        from tchuekam_cli.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_tchuekam_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from tchuekam_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run tchuekam chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'tchuekam setup' to configure[/]")
        except Exception:
    ...
    def _show_session_status(self):
        ...
        # 2.1.114's /recap.
        try:
            from tchuekam_cli.session_recap import build_recap
            recap = build_recap(
    ...
    def _fast_command_available(self) -> bool:
        try:
            from tchuekam_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from tchuekam_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from tchuekam_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with TchuEkaM!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in tchuekam_cli/colors.py
        ...
        from tchuekam_cli.tools_config import _get_platform_tools
        from tchuekam_cli.config import load_config
    ...
    def _handle_profile_command(self):
        ...
        from tchuekam_constants import display_tchuekam_home
        from tchuekam_cli.profiles import get_active_profile_name

        display = display_tchuekam_home()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from tchuekam_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("TCHUEKAM_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from tchuekam_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `tchuekam gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `tchuekam sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from tchuekam_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `tchuekam sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.tchuekam/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``tchuekam --resume <id>``
        ...
        saved_dir = get_tchuekam_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"tchuekam_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: tchuekam --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from tchuekam_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from tchuekam_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `tchuekam model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from tchuekam_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from tchuekam_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from tchuekam_cli.model_switch import switch_model, parse_model_flags
        from tchuekam_cli.providers import get_label
        ...
        # clobber disk config.
        from tchuekam_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — TchuEkaM default (chat_completions)
            /codex-runtime codex_app_server      — hand turns to codex subprocess
            /codex-runtime on / off              — synonyms for the above
        """
        from tchuekam_cli import codex_runtime_switch as crs
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from tchuekam_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to tchuekam_cli.curator so the CLI and the `tchuekam curator`
        ...
        try:
            from tchuekam_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from tchuekam_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to tchuekam_cli.skills_hub."""
        from tchuekam_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_tchuekam_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_tchuekam_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in tchuekam_cli/commands.py instead of touching every dispatch site.
        from tchuekam_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from tchuekam_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from tchuekam_cli.skin_engine import get_active_skin
            ...
            from tchuekam_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from tchuekam_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_tchuekam_home()}/plugins/ to get started.")
                else:
        ...
        elif canonical == "brief":
            from tools.tchuekam_briefing import generate_morning_brief
            cwd = os.getenv("TERMINAL_CWD", os.getcwd())
            _cprint(f"\n  {_DIM}[TCHUEKAM] Compiling executive daily brief for: {cwd}...{_RST}")
            try:
                brief = generate_morning_brief(cwd)
                print(brief)
            except Exception as e:
                _cprint(f"  {_DIM}✗ Briefing failed: {e}{_RST}")
        elif canonical == "search":
            from tools.tchuekam_indexer import query_index
            parts = cmd_original.split(None, 1)
            query_str = parts[1].strip() if len(parts) > 1 else ""
            if not query_str:
                _cprint("  Usage: /search <keywords>")
                return True
            _cprint(f"  {_DIM}[TCHUEKAM] Querying FTS5 index for '{query_str}'...{_RST}")
            res = query_index(query_str, limit=15)
            results = res.get("results", [])
            latency = res.get("latency_ms", 0.0)
            _cprint(f"  {_DIM}✓ Found {len(results)} matches in {latency:.2f}ms:{_RST}\n")
            for idx, r in enumerate(results, 1):
                _cprint(f"    [{idx}] {r['name']} ({r['size_kb']} KB)")
                _cprint(f"        Path: {r['path']}")
                _cprint(f"        Modified: {r['mtime']}")
            print()
        elif canonical == "index":
            from tools.tchuekam_indexer import run_background_index
            parts = cmd_original.split()
            paths = parts[1:] if len(parts) > 1 else None
            _cprint(f"  {_DIM}[TCHUEKAM] Launching background telemetry scan...{_RST}")
            run_background_index(paths)
            _cprint(f"  {_DIM}✓ Background index scan started.{_RST}")
        else:
            ...
            # Check for plugin-registered slash commands
                ...
                from tchuekam_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from tchuekam_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``tchuekam bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: tchuekam bundles create "
        ...
        _cprint(
            ...
            f"Manage with `tchuekam bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a TchuEkaM-managed isolated debug "
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from tchuekam_cli.goals import GoalManager
            from tchuekam_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"TchuEkaM keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from tchuekam_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_tchuekam_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from tchuekam_cli.config import load_config
        from tchuekam_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("TCHUEKAM_YOLO_MODE"))
        if current:
            os.environ.pop("TCHUEKAM_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while TchuEkaM is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while TchuEkaM is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while TchuEkaM is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from tchuekam_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from tchuekam_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update TchuEkaM Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``tchuekam update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from tchuekam_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update TchuEkaM Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update TchuEkaM Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update TchuEkaM Agent",
            detail="This will exit the current session and run `tchuekam update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by tchuekam_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from tchuekam_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.tchuekam/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_tchuekam_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "tchuekam_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _tchuekam_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  tchuekam --resume {self.session_id}")
            if session_title:
                print(f"  tchuekam -c \"{session_title}\"")
            print()
            ...
            try:
                from tchuekam_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from tchuekam_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from tchuekam_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        # TCHUEKAM-AGENT :: Boot sequence (cyberpunk startup experience)
        try:
            from tchuekam_cli.banner import print_boot_sequence
            print_boot_sequence(self._console)
        except Exception:
            pass  # Never break startup over a boot sequence error
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Tchuekam Agent! The First agentic AI solution developped in Cameroon. Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#b026ff")
        except Exception:
            _welcome_text = "Welcome to Tchuekam Agent! The First agentic AI solution developped in Cameroon. Type your message or /help for commands."
            _welcome_color = "#b026ff"
        ...
        try:
            _redact_raw = os.getenv("TCHUEKAM_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(TCHUEKAM_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→TchuEkaM migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from tchuekam_cli.config import get_config_path as _get_cfg_path_resid
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_tchuekam_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting TchuEkaM binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from tchuekam_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "TchuEkaM Agent")
        ...
        try:
            from tchuekam_cli.config import load_config
            from tchuekam_cli.voice import (
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if line_count >= 5 and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _tchuekam_home / "pastes"
        ...
        # Dynamic prompt: shows TchuEkaM symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("TchuEkaM needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'TCHUEKAM needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("TCHUEKAM needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = TchuEkaMCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_tchuekam_osd_patched", False):
                ...
                _pt_renderer._tchuekam_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``TCHUEKAM_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("TCHUEKAM_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the tchuekam process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: tchuekam setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                if getattr(self, '_delete_session_on_exit', False):
                    try:
                        from tchuekam_constants import get_tchuekam_home as _ghh
...
def main(
    ...
    """
    TchuEkaM Agent CLI - Interactive AI Assistant
    ...
    os.environ["TCHUEKAM_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting TchuEkaM Gateway (messaging platforms)...")
    ...
    # Default to tchuekam-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside TchuEkaMCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  TCHUEKAM_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "TchuEkaM" box, no tool-gen
            ...
            # Single-query mode (`tchuekam chat -q "…"`): skip the welcome
