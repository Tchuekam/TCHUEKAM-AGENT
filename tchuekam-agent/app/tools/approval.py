...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from tchuekam_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("TCHUEKAM_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("TCHUEKAM_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("TCHUEKAM_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set TCHUEKAM_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind TCHUEKAM_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds TCHUEKAM_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("TCHUEKAM_CRON_SESSION"):
        return False
    if env_var_enabled("TCHUEKAM_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $TCHUEKAM_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_TCHUEKAM_ENV_PATH = (
    r'(?:~\/\.tchuekam/|'
    r'(?:\$home|\$\{home\})/\.tchuekam/|'
    r'(?:\$tchuekam_home|\$\{tchuekam_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\btchuekam\s+gateway\s+(stop|restart)\b', "stop/restart tchuekam gateway (kills running agents)"),
    (r'\btchuekam\s+update\b', "tchuekam update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart tchuekam-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart tchuekam-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(tchuekam|gateway|cli\.py)\b', "kill tchuekam/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill tchuekam` but not
    # `kill -9 $(pgrep -f tchuekam)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["TCHUEKAM_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "TCHUEKAM_SPINNER_PAUSE" in os.environ:
            del os.environ["TCHUEKAM_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("TCHUEKAM_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("TCHUEKAM_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("TCHUEKAM_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("TCHUEKAM_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("TCHUEKAM_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("TCHUEKAM_EXEC_ASK")
