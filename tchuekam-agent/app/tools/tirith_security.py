...
it is automatically downloaded from GitHub releases to $TCHUEKAM_HOME/bin/tirith.
...
def _get_tchuekam_home() -> str:
    """Return the TchuEkaM home directory, respecting TCHUEKAM_HOME env var."""
    return str(get_tchuekam_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_tchuekam_home(), ".tirith-install-failed")
...
def _tchuekam_bin_dir() -> str:
    """Return $TCHUEKAM_HOME/bin, creating it if needed."""
    d = os.path.join(_get_tchuekam_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TCHUEKAM_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_tchuekam_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TCHUEKAM_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TCHUEKAM_HOME/bin/tirith
    ...
    tchuekam_bin = os.path.join(_tchuekam_bin_dir(), "tirith")
    if os.path.isfile(tchuekam_bin) and os.access(tchuekam_bin, os.X_OK):
        _resolved_path = tchuekam_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return tchuekam_bin
