...
def _load_tchuekam_env_vars() -> dict[str, str]:
    """Load ~/.tchuekam/.env values without failing Docker command execution."""
    try:
        from tchuekam_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``TCHUEKAM_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("TCHUEKAM_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using TCHUEKAM_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'tchuekam'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.tchuekam/sandboxes/). Non-persistent
        ...
        container_name = f"tchuekam-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic TchuEkaM secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _TCHUEKAM_PROVIDER_ENV_BLOCKLIST)
        tchuekam_env = _load_tchuekam_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = tchuekam_env.get(key)
