...
One ``CDPSupervisor`` runs per TchuEkaM ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "tchuekam-dialog-bridge.invalid"
...
(() => {
  if (window.__tchuekamDialogBridgeInstalled) return;
  window.__tchuekamDialogBridgeInstalled = true;
  const ENDPOINT = "http://tchuekam-dialog-bridge.invalid/";
