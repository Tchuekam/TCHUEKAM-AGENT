...
from tools.tchuekam_indexer import query_index, run_background_index
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    another TchuEkaM profile's skills/plugins/cron/memories directory.

    Returns ``None`` when the write is in-scope (same profile) or outside
    TchuEkaM scope entirely. Soft guard — the agent can override by passing
    ...
    # in a session that cd'd into ``~/.tchuekam/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── TchuEkaM internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under TCHUEKAM_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == TCHUEKAM_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-TchuEkaM-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-TchuEkaM-profile guard for
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
    ...
    try:
        ...
        # ── TCHUEKAM high-speed database / memory index check ──
        if target == "files":
            clean_pattern = pattern.replace("*", "").replace("?", "").strip()
            if clean_pattern:
                idx_res = query_index(clean_pattern, limit=limit)
                if idx_res and idx_res.get("results"):
                    matches = []
                    for r in idx_res["results"]:
                        if path and path != ".":
                            try:
                                full_search_path = str(Path(path).resolve())
                                full_item_path = str(Path(r["path"]).resolve())
                                if not full_item_path.startswith(full_search_path):
                                    continue
                            except Exception:
                                pass
                        matches.append({
        ...
        if target == "files" and result_dict.get("matches"):
            try:
                from tools.tchuekam_indexer import save_to_search_memory
                found_files = []
                for m in result_dict["matches"]:
                    found_files.append({
                        "path": m["path"],
                        "name": m.get("name", os.path.basename(m["path"]))
                    })
                save_to_search_memory(found_files)
            except Exception:
                pass
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another TchuEkaM profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another TchuEkaM profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with tchuekam_tools.write_file() for very "
            "large files."
        )
...
TCHUEKAM_INDEX_SEARCH_SCHEMA = {
    "name": "tchuekam_index_search",
    "description": "Ultra-fast database-backed keyword search across all local drives (C:, D:, H:). Resolves in <50ms. Searches file names and paths instantly using an offline SQLite FTS5 index. Use this instead of search_files when you need to locate files instantly across the entire system.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {"type": "string", "description": "The search term, filename keyword, or path fragment to locate."},
...
def _handle_tchuekam_index_search(args, **kw):
    query = args.get("query", "")
    limit = args.get("limit", 50)
    res = query_index(query, limit)
...
registry.register(name="tchuekam_index_search", toolset="file", schema=TCHUEKAM_INDEX_SEARCH_SCHEMA, handler=_handle_tchuekam_index_search, check_fn=_check_file_reqs, emoji="⚡", max_result_size_chars=100_000)

# Start the TCHUEKAM high-speed file indexer in the background on import
try:
    run_background_index()
except Exception as e:
    logger.debug("Failed to start TCHUEKAM background index scan", exc_info=True)
