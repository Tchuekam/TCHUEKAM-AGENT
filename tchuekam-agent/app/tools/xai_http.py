...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``tchuekam tools`` repaint, tool-registration scans,
    ...
    2. ``~/.tchuekam/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_tchuekam_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.tchuekam/.env`` first, then ``os.environ``.

    Wraps :func:`tchuekam_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from tchuekam_cli.config import get_env_value as _tchuekam_get_env_value

        value = _tchuekam_get_env_value(name)
...
def tchuekam_xai_user_agent() -> str:
    """Return a stable TchuEkaM-specific User-Agent for xAI HTTP calls."""
    try:
        from tchuekam_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"TchuEkaM-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers TchuEkaM-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``tchuekam_cli.config.get_env_value`` so keys
    stored in ``~/.tchuekam/.env`` (the standard TchuEkaM location) are honored —
