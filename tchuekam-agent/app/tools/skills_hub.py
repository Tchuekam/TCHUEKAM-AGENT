#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the TchuEkaM Skills Hub.
...
Used by tchuekam_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            tchuekam_meta = metadata.get("tchuekam", {})
            if isinstance(tchuekam_meta, dict):
                tags = tchuekam_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            tchuekam_meta = metadata.get("tchuekam", {})
            if isinstance(tchuekam_meta, dict):
                raw_tags = tchuekam_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.tchuekam/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                tchuekam_meta = meta_block.get("tchuekam", {})
                if isinstance(tchuekam_meta, dict):
                    tags = tchuekam_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# TchuEkaM centralized index source
# ---------------------------------------------------------------------------

TCHUEKAM_INDEX_URL = "https://tchuekam-agent.nousresearch.com/docs/api/skills-index.json"
TCHUEKAM_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "tchuekam-index.json"
TCHUEKAM_INDEX_TTL = 6 * 3600  # 6 hours


def _load_tchuekam_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for TCHUEKAM_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if TCHUEKAM_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - TCHUEKAM_INDEX_CACHE_FILE.stat().st_mtime
            if age < TCHUEKAM_INDEX_TTL:
                return json.loads(TCHUEKAM_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(TCHUEKAM_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("TchuEkaM index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("TchuEkaM index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        TCHUEKAM_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        TCHUEKAM_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class TchuEkaMIndexSource(SkillSource):
    """Skill source backed by the centralized TchuEkaM Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_tchuekam_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "tchuekam-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "tchuekam-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        TchuEkaMIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "tchuekam-index"
