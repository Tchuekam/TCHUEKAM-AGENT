...
Lets the LLM write a Python script that calls TchuEkaM tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `tchuekam_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `tchuekam_tools.py` with file-based RPC stubs
...
# available on every platform TchuEkaM itself runs on.
...
# ---------------------------------------------------------------------------
# tchuekam_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-TchuEkaM-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-TchuEkaM-profile soft guard."""',
...
def generate_tchuekam_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the tchuekam_tools.py stub module.
...
"""Auto-generated TchuEkaM tools RPC stubs."""
...
def _connect():
    ...
    TCHUEKAM_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["TCHUEKAM_RPC_SOCKET"]
...
"""Auto-generated TchuEkaM tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("TCHUEKAM_RPC_DIR") or os.path.join(tempfile.gettempdir(), "tchuekam_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated tchuekam_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/tchuekam_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_tchuekam_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/tchuekam_tools.py", tools_src)
        ...
        env_prefix = (
            f"TCHUEKAM_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("TCHUEKAM_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with tchuekam_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="tchuekam_sandbox_")
    ...
    # selector from TCHUEKAM_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"tchuekam_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated tchuekam_tools module.
        ...
        tools_src = generate_tchuekam_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "tchuekam_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  TCHUEKAM_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["TCHUEKAM_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the tchuekam-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from tchuekam_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _tchuekam_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _tchuekam_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # TCHUEKAM_TIMEZONE is an internal TchuEkaM setting and must not leak
        # into child processes.
        _tz_name = os.getenv("TCHUEKAM_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("TCHUEKAM_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {TCHUEKAM_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.tchuekam/.env')).
...
def _load_config() -> dict:
    ...
    ``tchuekam --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (tchuekam-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``tchuekam tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and tchuekam-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.tchuekam/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call TchuEkaM tools programmatically. "
        ...
        f"Available via `from tchuekam_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into tchuekam_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from tchuekam_tools import {import_str}` "
