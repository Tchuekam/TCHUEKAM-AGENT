...
VERSION_FILE = REPO_ROOT / "tchuekam_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+tchuekam-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "tchuekam@nousresearch.com": "NousResearch",
    ...
    "tchuekam@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@tchuekam-agent": "teknium1",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (TCHUEKAM_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (TCHUEKAM_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: tchuekam-eval community link)
    ...
    "tchuekamagent26@gmail.com": "tchuekamagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+tchuekamagent26@users.noreply.github.com": "tchuekamagent26",
    ...
    "vigo@tchuekam": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.tchuekam@agentmail.to": "oseftg",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"tchuekam-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "tchuekam@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/tchuekam-agent",
    ...
    lines.append(f"# TchuEkaM Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for TchuEkaM Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="TchuEkaM Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  TchuEkaM Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"TchuEkaM Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"TchuEkaM Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'TchuEkaM Agent v{new_version} ({calver_date})' "
