...
# Bootstrap Open WebUI against TchuEkaM Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.tchuekam/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-tchuekam.sh
...
#   OPEN_WEBUI_NAME='Johnny TchuEkaM'
...
#   TCHUEKAM_API_PORT=8642
#   TCHUEKAM_API_HOST=127.0.0.1
#   TCHUEKAM_API_MODEL_NAME='TchuEkaM Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-TchuEkaM Agent WebUI}"
...
TCHUEKAM_ENV_FILE="${TCHUEKAM_ENV_FILE:-$HOME/.tchuekam/.env}"
TCHUEKAM_API_PORT="${TCHUEKAM_API_PORT:-8642}"
TCHUEKAM_API_HOST="${TCHUEKAM_API_HOST:-127.0.0.1}"
TCHUEKAM_API_CONNECT_HOST="${TCHUEKAM_API_CONNECT_HOST:-127.0.0.1}"
TCHUEKAM_API_MODEL_NAME="${TCHUEKAM_API_MODEL_NAME:-TchuEkaM Agent}"
TCHUEKAM_API_BASE_URL="http://${TCHUEKAM_API_CONNECT_HOST}:${TCHUEKAM_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-tchuekam.sh"
LOG_DIR="$HOME/.tchuekam/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$TCHUEKAM_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$TCHUEKAM_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.tchuekam.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.tchuekam</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.tchuekam"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.tchuekam"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-tchuekam.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to TchuEkaM Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-tchuekam.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.tchuekam/logs/openwebui.log
StandardError=append:%h/.tchuekam/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-tchuekam.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$TCHUEKAM_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring TchuEkaM API server is configured...'
  upsert_env API_SERVER_ENABLED true "$TCHUEKAM_ENV_FILE"
  upsert_env API_SERVER_HOST "$TCHUEKAM_API_HOST" "$TCHUEKAM_ENV_FILE"
  upsert_env API_SERVER_PORT "$TCHUEKAM_API_PORT" "$TCHUEKAM_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$TCHUEKAM_API_MODEL_NAME" "$TCHUEKAM_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$TCHUEKAM_ENV_FILE"
  ensure_env_permissions

  log 'Restarting TchuEkaM gateway so API server settings take effect...'
  tchuekam gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${TCHUEKAM_API_CONNECT_HOST}:${TCHUEKAM_API_PORT}/health" >/dev/null; then
    log 'TchuEkaM API server did not answer on the first check. Trying to start gateway in the background...'
    nohup tchuekam gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${TCHUEKAM_API_CONNECT_HOST}:${TCHUEKAM_API_PORT}/health" >/dev/null
  ...
  log "TchuEkaM API endpoint: ${TCHUEKAM_API_BASE_URL}"
