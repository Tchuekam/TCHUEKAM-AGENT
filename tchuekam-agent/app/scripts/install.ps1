# ============================================================================
# TchuEkaM Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$TchuEkaMHome = "$env:LOCALAPPDATA\tchuekam",
    [string]$InstallDir = "$env:LOCALAPPDATA\tchuekam\tchuekam-agent",
...
$RepoUrlSsh = "git@github.com:NousResearch/tchuekam-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/tchuekam-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * TchuEkaM Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $TchuEkaMHome)) {
        New-Item -ItemType Directory -Force -Path $TchuEkaMHome | Out-Null
    }
    $envFile = Join-Path $TchuEkaMHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $TchuEkaMHome "node"
...
function Install-Git {
    ...
    which TchuEkaM uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\tchuekam\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    TchuEkaM needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the TchuEkaM copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\tchuekam\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``TCHUEKAM_GIT_BASH_PATH`` (User scope) so TchuEkaM can find it in a fresh
    ...
    # Download PortableGit into $TchuEkaMHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $TchuEkaMHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent TchuEkaM features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  TchuEkaM needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``TCHUEKAM_GIT_BASH_PATH`` (User env scope) so TchuEkaM can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $TchuEkaMHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $TchuEkaMHome\git\usr\bin\bash.exe
    $candidates += "$TchuEkaMHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$TchuEkaMHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("TCHUEKAM_GIT_BASH_PATH", $candidate, "User")
            $env:TCHUEKAM_GIT_BASH_PATH = $candidate
            Write-Info "Set TCHUEKAM_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- TchuEkaM may not find Git Bash."
    Write-Info "If needed, set TCHUEKAM_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$TchuEkaMHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$TchuEkaMHome\node;$env:Path"
        Write-Success "Node.js $version found (TchuEkaM-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $TchuEkaMHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $TchuEkaMHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\tchuekam-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$TchuEkaMHome\node") { Remove-Item -Recurse -Force "$TchuEkaMHome\node" }
                Move-Item $extractedDir.FullName "$TchuEkaMHome\node"
                ...
                $version = & "$TchuEkaMHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $TchuEkaMHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running tchuekam processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/tchuekam-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/tchuekam-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/tchuekam-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\tchuekam-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\tchuekam-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `tchuekam.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install tchuekam-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and tchuekam.exe broken with
    ...
    # here, not 30 seconds later when the user runs `tchuekam`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `tchuekam dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `tchuekam dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `tchuekam dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up tchuekam command..."
    
    if ($NoVenv) {
        $tchuekamBin = "$InstallDir"
    } else {
        $tchuekamBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so tchuekam is globally available
    # On Windows, the tchuekam.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$tchuekamBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$tchuekamBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $tchuekamBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set TCHUEKAM_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\tchuekam instead
    # of the Unix default ~/.tchuekam
    $currentTchuEkaMHome = [Environment]::GetEnvironmentVariable("TCHUEKAM_HOME", "User")
    if (-not $currentTchuEkaMHome -or $currentTchuEkaMHome -ne $TchuEkaMHome) {
        [Environment]::SetEnvironmentVariable("TCHUEKAM_HOME", $TchuEkaMHome, "User")
        Write-Success "Set TCHUEKAM_HOME=$TchuEkaMHome"
    }
    $env:TCHUEKAM_HOME = $TchuEkaMHome
    
    # Update current session
    $env:Path = "$tchuekamBin;$env:Path"
    
    Write-Success "tchuekam command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.tchuekam directory structure
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$TchuEkaMHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$TchuEkaMHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.tchuekam/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.tchuekam/.env"
        }
    } else {
        Write-Info "~/.tchuekam/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$TchuEkaMHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.tchuekam/config.yaml from template"
        }
    } else {
        Write-Info "~/.tchuekam/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and TchuEkaM's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.tchuekam/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.tchuekam/"
    
    # Seed bundled skills into ~/.tchuekam/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.tchuekam/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.tchuekam/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$TchuEkaMHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.tchuekam/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'tchuekam setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\tchuekam-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\tchuekam-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\tchuekam-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.tchuekam/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'tchuekam setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m tchuekam_cli.main setup
    } else {
        python -m tchuekam_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $tchuekamCmd = "$InstallDir\venv\Scripts\tchuekam.exe"
    if (-not (Test-Path $tchuekamCmd)) {
        $tchuekamCmd = "tchuekam"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$TchuEkaMHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'tchuekam whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: tchuekam gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$TchuEkaMHome\logs\gateway.log"
            Start-Process -FilePath $tchuekamCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$TchuEkaMHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: tchuekam gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: tchuekam gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$TchuEkaMHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$TchuEkaMHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TchuEkaMHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$TchuEkaMHome\tchuekam-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   tchuekam              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   tchuekam setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   tchuekam config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   tchuekam config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   tchuekam gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   tchuekam update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning TchuEkaM repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding TchuEkaM to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/tchuekam-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
