"""
TchuEkaM MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (TchuEkaM-specific extra)

Usage:
    tchuekam mcp serve
    tchuekam mcp serve --verbose
...
logger = logging.getLogger("tchuekam.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using TCHUEKAM_HOME."""
    try:
        from tchuekam_constants import get_tchuekam_home
        return get_tchuekam_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("TCHUEKAM_HOME", Path.home() / ".tchuekam")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from tchuekam_constants import get_tchuekam_home
        directory_file = get_tchuekam_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("TCHUEKAM_HOME", Path.home() / ".tchuekam")
...
class EventBridge:
    ...
    This is the TchuEkaM equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from tchuekam_constants import get_tchuekam_home
            db_file = get_tchuekam_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("TCHUEKAM_HOME", Path.home() / ".tchuekam")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the TchuEkaM MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "tchuekam",
        instructions=(
            "TchuEkaM Agent messaging bridge. Use these tools to interact with "
