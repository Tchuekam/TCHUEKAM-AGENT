...
await build({
  ...
  // Skip the prebuilt @tchuekam/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@tchuekam/ink': resolve(root, 'packages/tchuekam-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The tchuekam_cli launcher
