import type { RunExternalProcess } from '@tchuekam/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`tchuekam ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching tchuekam: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`tchuekam ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
