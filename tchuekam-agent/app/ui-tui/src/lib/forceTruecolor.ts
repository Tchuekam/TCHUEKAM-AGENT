...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.TCHUEKAM_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep TchuEkaM on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via TCHUEKAM_TUI_TRUECOLOR=1.
