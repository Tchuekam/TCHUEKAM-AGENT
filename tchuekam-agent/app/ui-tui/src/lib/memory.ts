...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.TCHUEKAM_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.tchuekam', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `tchuekam-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
