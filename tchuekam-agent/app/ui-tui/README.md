...
React + Ink terminal UI for TchuEkaM. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `TCHUEKAM_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.tchuekam/.tchuekam_history` or under `TCHUEKAM_HOME`.
...
```text
ui-tui/
  packages/tchuekam-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      tchuekam-ink.d.ts    type declarations for @tchuekam/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent TchuEkaMCLI subprocess for slash commands
```
