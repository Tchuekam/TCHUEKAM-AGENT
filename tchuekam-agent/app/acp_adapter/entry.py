"""CLI entry point for the tchuekam-agent ACP adapter.

Loads environment variables from ``~/.tchuekam/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from TCHUEKAM_HOME (default ``~/.tchuekam``)."""
    from tchuekam_cli.env_loader import load_tchuekam_dotenv

    tchuekam_home = get_tchuekam_home()
    loaded = load_tchuekam_dotenv(tchuekam_home=tchuekam_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", tchuekam_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="tchuekam-acp",
        description="Run TchuEkaM Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print TchuEkaM version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(tchuekam_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import TchuEkaMACPAgent  # noqa: F401

    print("TchuEkaM ACP check OK")


def _run_setup() -> None:
    from tchuekam_cli.main import main as tchuekam_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "tchuekam", "model"]
        tchuekam_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``tchuekam postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting tchuekam-agent ACP adapter")
    ...
    import acp
    from .server import TchuEkaMACPAgent
