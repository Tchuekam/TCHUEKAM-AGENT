"""ACP auth helpers — detect and advertise TchuEkaM authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "tchuekam-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active TchuEkaM runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if TchuEkaM can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for TchuEkaM.
    ...
    not have TchuEkaM provider credentials configured yet, so TchuEkaM always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate TchuEkaM using the currently configured "
