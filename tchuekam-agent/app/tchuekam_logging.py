"""Centralized logging setup for TchuEkaM Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.tchuekam/logs/`` (profile-aware via ``get_tchuekam_home()``).
...
from tchuekam_constants import get_config_path, get_tchuekam_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_tchuekam_session_injector", False):
    ...
    _session_record_factory._tchuekam_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``tchuekam logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "tchuekam_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("tchuekam_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the TchuEkaM logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    tchuekam_home
        Override for the TchuEkaM home directory.  Falls back to
        ``get_tchuekam_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_tchuekam_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._tchuekam_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the tchuekam group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from tchuekam_cli.config import is_managed
