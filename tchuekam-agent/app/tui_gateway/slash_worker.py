"""Persistent slash-command worker — one TchuEkaMCLI per TUI session.
...
def _run(cli: TchuEkaMCLI, command: str) -> str:
...
def main():
    ...
    os.environ["TCHUEKAM_SESSION_KEY"] = args.session_key
    os.environ["TCHUEKAM_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = TchuEkaMCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
