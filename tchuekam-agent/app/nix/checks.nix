...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      tchuekam-agent = self'.packages.default;
      tchuekamVenv = tchuekam-agent.tchuekamVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "tchuekam-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${tchuekamVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "tchuekam-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "tchuekam-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${tchuekam-agent}/bin/tchuekam || (echo "FAIL: tchuekam binary missing"; exit 1)
          test -x ${tchuekam-agent}/bin/tchuekam-agent || (echo "FAIL: tchuekam-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${tchuekam-agent}/bin/tchuekam version 2>&1 | grep -qi "tchuekam" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "tchuekam-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in tchuekam tchuekam-agent tchuekam-acp; do
            test -x ${tchuekam-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "tchuekam-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking tchuekam --help ==="
          ${tchuekam-agent}/bin/tchuekam --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${tchuekam-agent}/bin/tchuekam --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "tchuekam-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${tchuekam-agent}/share/tchuekam-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${tchuekam-agent}/share/tchuekam-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "TCHUEKAM_BUNDLED_SKILLS" ${tchuekam-agent}/bin/tchuekam || \
            (echo "FAIL: TCHUEKAM_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: TCHUEKAM_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "tchuekam-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${tchuekam-agent}/share/tchuekam-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${tchuekam-agent}/share/tchuekam-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "TCHUEKAM_BUNDLED_PLUGINS" ${tchuekam-agent}/bin/tchuekam || \
            (echo "FAIL: TCHUEKAM_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: TCHUEKAM_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "tchuekam-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${tchuekam-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${tchuekam-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "TCHUEKAM_TUI_DIR" ${tchuekam-agent}/bin/tchuekam || \
            (echo "FAIL: TCHUEKAM_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: TCHUEKAM_TUI_DIR set in wrapper"
        ...
        # Verify TCHUEKAM_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        tchuekam-node = pkgs.runCommand "tchuekam-node-version" { } ''
          set -e
          echo "=== Checking TCHUEKAM_NODE in wrapper ==="
          grep -q "TCHUEKAM_NODE" ${tchuekam-agent}/bin/tchuekam || \
            (echo "FAIL: TCHUEKAM_NODE not set in wrapper"; exit 1)
          echo "PASS: TCHUEKAM_NODE present in wrapper"

          TCHUEKAM_NODE=$(sed -n "s/^export TCHUEKAM_NODE='\(.*\)'/\1/p" ${tchuekam-agent}/bin/tchuekam)
          test -x "$TCHUEKAM_NODE" || (echo "FAIL: TCHUEKAM_NODE=$TCHUEKAM_NODE not executable"; exit 1)
          echo "PASS: TCHUEKAM_NODE executable at $TCHUEKAM_NODE"

          NODE_MAJOR=$("$TCHUEKAM_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All TCHUEKAM_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify TCHUEKAM_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "tchuekam-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(TCHUEKAM_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking TCHUEKAM_MANAGED guards ==="
          check_blocked "config set" ${tchuekam-agent}/bin/tchuekam config set model foo
          check_blocked "config edit" ${tchuekam-agent}/bin/tchuekam config edit
        ...
        extra-python-packages = let
          ...
          tchuekamWithExtra = tchuekam-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "tchuekam-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${tchuekamWithExtra}/bin/tchuekam || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${tchuekamWithExtra}/bin/tchuekam || \
          ...
          if grep -q "PYTHONPATH" ${tchuekam-agent}/bin/tchuekam; then
        ...
        extra-dependency-groups = let
          tchuekamWithGroups = tchuekam-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "tchuekam-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${tchuekamWithGroups}"
          echo "venv: ${tchuekamWithGroups.tchuekamVenv}"
        ...
        in pkgs.runCommand "tchuekam-config-roundtrip" {
          ...
          merge_and_load() {
            local tchuekam_home="$1"
            export TCHUEKAM_HOME="$tchuekam_home"
            ${configMergeScript} ${nixSettings} "$tchuekam_home/config.yaml"
            ${tchuekamVenv}/bin/python3 -c '
import json, sys
