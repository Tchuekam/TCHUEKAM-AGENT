# nix/nixosModules.nix — NixOS module for tchuekam-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: tchuekam runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $TCHUEKAM_HOME/.env
# and read by tchuekam at startup — no container recreation needed for env changes.
#
# Tool resolution: the tchuekam wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.tchuekam-agent;
    effectivePackage =
    ...
    tchuekam-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-tchuekam-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "tchuekam-config-attrs";
      description = "TchuEkaM YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "tchuekam-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "tchuekam-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'TCHUEKAM_DOC_EOF'\n${value}\nTCHUEKAM_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "tchuekam-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/tchuekam";
    ...
    # tchuekam user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "tchuekam-container-entrypoint" ''
      set -eu

      TCHUEKAM_UID="''${TCHUEKAM_UID:?TCHUEKAM_UID must be set}"
      TCHUEKAM_GID="''${TCHUEKAM_GID:?TCHUEKAM_GID must be set}"

      # ── Group: ensure a group with GID=$TCHUEKAM_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$TCHUEKAM_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="tchuekam"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$TCHUEKAM_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$TCHUEKAM_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$TCHUEKAM_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$TCHUEKAM_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="tchuekam"
        TARGET_HOME="/home/tchuekam"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$TCHUEKAM_UID" -g "$TCHUEKAM_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$TCHUEKAM_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$TCHUEKAM_UID:$TCHUEKAM_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure TCHUEKAM_HOME is owned by the target user.
      ...
      if [ -n "''${TCHUEKAM_HOME:-}" ] && [ -d "$TCHUEKAM_HOME" ]; then
        find "$TCHUEKAM_HOME" \! -user "$TCHUEKAM_UID" -exec chown "$TCHUEKAM_UID:$TCHUEKAM_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/tchuekam-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/tchuekam-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/tchuekam ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/tchuekam
        chmod 0440 /etc/sudoers.d/tchuekam
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$TCHUEKAM_UID" --regid="$TCHUEKAM_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $TCHUEKAM_HOME/.env.
    ...
    # Default: /var/lib/tchuekam/workspace → /data/workspace.
  ...
  in {
    options.services.tchuekam-agent = with lib; {
      enable = mkEnableOption "TchuEkaM Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = tchuekam-agent;
        description = "The tchuekam-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/tchuekam";
        description = "State directory. Contains .tchuekam/ subdir (TCHUEKAM_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative TchuEkaM config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $TCHUEKAM_HOME/.env at activation time.
          TchuEkaM reads this file on every startup via load_tchuekam_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $TCHUEKAM_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $TCHUEKAM_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `tchuekam gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the tchuekam user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the tchuekam plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. TchuEkaM discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          tchuekam_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as tchuekam (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in tchuekam-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the tchuekam CLI to environment.systemPackages and export
          TCHUEKAM_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.tchuekam symlink to the service
            stateDir. These users are automatically added to the tchuekam group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.tchuekam-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the tchuekam CLI to system PATH and export TCHUEKAM_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.tchuekam/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.TCHUEKAM_HOME = "${cfg.stateDir}/.tchuekam";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.tchuekam-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the tchuekam user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.tchuekam-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed tchuekam
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure tchuekam is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.tchuekam        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tchuekam/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tchuekam/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tchuekam/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tchuekam/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.tchuekam/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."tchuekam-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.tchuekam
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.tchuekam ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.tchuekam ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.tchuekam -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.tchuekam/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.tchuekam/$_subdir"
            chmod 2770 "${cfg.stateDir}/.tchuekam/$_subdir"
            find "${cfg.stateDir}/.tchuekam/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.tchuekam/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.tchuekam/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.tchuekam/config.yaml
            chmod 0640 ${cfg.stateDir}/.tchuekam/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.tchuekam/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.tchuekam/.managed
          chmod 0644 ${cfg.stateDir}/.tchuekam/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.tchuekam/.container-mode <<'TCHUEKAM_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    tchuekam_bin=${containerDataDir}/current-package/bin/tchuekam
    TCHUEKAM_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.tchuekam/.container-mode
            chmod 0644 ${cfg.stateDir}/.tchuekam/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.tchuekam/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.tchuekam";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.tchuekam" ]; then
                  rm -f "${symlinkPath}"
                  echo "tchuekam-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.tchuekam -> stateDir/.tchuekam for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.tchuekam";
                target = "${cfg.stateDir}/.tchuekam";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "tchuekam-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.tchuekam/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.tchuekam/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.tchuekam/auth.json
              fi
            ''}
          ''}
          ...
          # TchuEkaM reads $TCHUEKAM_HOME/.env at startup via load_tchuekam_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.tchuekam/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'TCHUEKAM_NIX_ENV_EOF'
    ${envFileContent}
    TCHUEKAM_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.tchuekam/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.tchuekam/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.tchuekam/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.tchuekam-agent = {
          description = "TchuEkaM Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            TCHUEKAM_HOME = "${cfg.stateDir}/.tchuekam";
            TCHUEKAM_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $TCHUEKAM_HOME/.env by the activation script. load_tchuekam_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/tchuekam"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the tchuekam group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.tchuekam-agent = {
          description = "TchuEkaM Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              TCHUEKAM_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              TCHUEKAM_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env TCHUEKAM_UID="$TCHUEKAM_UID" \
                --env TCHUEKAM_GID="$TCHUEKAM_GID" \
                --env TCHUEKAM_HOME=${containerDataDir}/.tchuekam \
                --env TCHUEKAM_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/tchuekam gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
