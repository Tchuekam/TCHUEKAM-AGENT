# TchuEkaM Agent Has Had "Routines" Since March
...
TchuEkaM equivalent — works today:
```bash
...
TchuEkaM equivalent — works today:
```bash
tchuekam webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working TchuEkaM implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | TchuEkaM Agent |
|---|---|---|
...
## Things TchuEkaM Does That Routines Can't
...
TchuEkaM has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
TchuEkaM Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
tchuekam gateway setup    # enable webhooks
tchuekam webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [tchuekam-agent.nousresearch.com/docs/guides/automation-templates](https://tchuekam-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [tchuekam-agent.nousresearch.com](https://tchuekam-agent.nousresearch.com)

GitHub: [github.com/NousResearch/tchuekam-agent](https://github.com/NousResearch/tchuekam-agent)

---

*TchuEkaM Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
