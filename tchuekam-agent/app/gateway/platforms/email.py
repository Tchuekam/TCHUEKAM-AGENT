"""
Email platform adapter for the TchuEkaM gateway.

Allows users to interact with TchuEkaM by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "tchuekam-agent" "version" "{_tchuekam_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "TchuEkaM Agent")
        ...
        msg_id = f"<tchuekam-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
