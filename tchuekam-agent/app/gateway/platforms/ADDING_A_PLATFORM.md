...
There are two ways to add a platform to the TchuEkaM gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.tchuekam/plugins/` (or under `plugins/platforms/`
...
**zero changes to core TchuEkaM code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `tchuekam gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `tchuekam_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the TchuEkaM core.
...
And add it to the `tchuekam-gateway` composite:

```python
"tchuekam-gateway": {
    "includes": [..., "tchuekam-your-platform"]
}
```
...
## 12. Status Display (`tchuekam_cli/status.py`)
...
## 13. Gateway Setup Wizard (`tchuekam_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ tchuekam_cli/ toolsets.py \
  --include="*.py" -l | sort -u
