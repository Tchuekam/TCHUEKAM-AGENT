...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.tchuekam/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. TchuEkaM upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "tchuekam-agent",
                    "$device": "tchuekam-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.tchuekam/.update_response`` for the
          detached ``tchuekam update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``tchuekam update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `tchuekam setup gateway` / `tchuekam gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``tchuekam update --gateway`` watcher. Button
        ...
        ``~/.tchuekam/.update_response`` so the detached update process
        can read it.
        """
