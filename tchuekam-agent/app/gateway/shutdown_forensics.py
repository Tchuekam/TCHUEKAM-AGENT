    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if tchuekam_home_str:
            takeover_path = Path(tchuekam_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(tchuekam_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded tchuekam-agent but never re-ran ``tchuekam setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../tchuekam-gateway.service"
    ...
    # that's the common case for tchuekam.
    ...
    # constant in tchuekam_cli/gateway.py.
    headroom = 30.0
