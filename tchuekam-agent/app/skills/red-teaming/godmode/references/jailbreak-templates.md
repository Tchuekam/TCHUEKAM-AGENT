...
## 5. GODMODE FAST — TchuEkaM 4 405B (Zero Refusal)

**Model:** `nousresearch/tchuekam-4-405b`
**Strategy:** TchuEkaM models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with TchuEkaM Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.tchuekam/config.yaml`:
