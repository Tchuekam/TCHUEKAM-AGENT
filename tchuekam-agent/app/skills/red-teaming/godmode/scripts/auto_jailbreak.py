...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("TCHUEKAM_HOME", os.path.expanduser("~/.tchuekam")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("TCHUEKAM_HOME", Path.home() / ".tchuekam")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# TchuEkaM config paths
# ═══════════════════════════════════════════════════════════════════

TCHUEKAM_HOME = Path(os.getenv("TCHUEKAM_HOME", Path.home() / ".tchuekam"))
CONFIG_PATH = TCHUEKAM_HOME / "config.yaml"
PREFILL_PATH = TCHUEKAM_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/TchuEkaM models — already uncensored, just needs clean prompt
    "tchuekam": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "tchuekam" in model_lower or "nous" in model_lower:
        return "tchuekam"
...
def _get_current_model() -> tuple:
    """Read current model and provider from TchuEkaM config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.tchuekam/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart TchuEkaM for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart TchuEkaM for changes to take effect.")
