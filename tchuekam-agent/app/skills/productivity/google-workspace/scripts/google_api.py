#!/usr/bin/env python3
"""Google Workspace API CLI for TchuEkaM Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing TchuEkaM-facing JSON contract and falls back to the Python client
...
from _tchuekam_home import get_tchuekam_home
...
def _gws_binary() -> str | None:
    override = os.getenv("TCHUEKAM_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for TchuEkaM Agent")
