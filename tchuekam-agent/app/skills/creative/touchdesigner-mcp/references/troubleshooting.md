...
## 5. TchuEkaM Configuration

### Config location

`$TCHUEKAM_HOME/config.yaml` (defaults to `~/.tchuekam/config.yaml` when `TCHUEKAM_HOME` is unset)
...
Restart the TchuEkaM session for changes to take effect. The MCP connection is
