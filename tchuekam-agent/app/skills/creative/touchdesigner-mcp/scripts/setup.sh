...
TCHUEKAM_HOME_DIR="${TCHUEKAM_HOME:-$HOME/.tchuekam}"
TCHUEKAM_CFG="${TCHUEKAM_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure TchuEkaM config has twozero_td MCP entry ──
if [[ ! -f "$TCHUEKAM_CFG" ]]; then
    echo -e " ${FAIL} TchuEkaM config not found at ${TCHUEKAM_CFG}"
    manual_steps+=("Create ${TCHUEKAM_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$TCHUEKAM_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in TchuEkaM config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to TchuEkaM config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$TCHUEKAM_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${TCHUEKAM_CFG} manually"); }
    manual_steps+=("Restart TchuEkaM session to pick up config change")
fi
