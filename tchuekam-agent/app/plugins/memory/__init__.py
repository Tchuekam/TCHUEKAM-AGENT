...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with tchuekam-agent)
2. User-installed providers: ``$TCHUEKAM_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$TCHUEKAM_HOME/plugins/`` or None if unavailable."""
    try:
        from tchuekam_constants import get_tchuekam_home
        d = get_tchuekam_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$TCHUEKAM_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_tchuekam_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_tchuekam_user_memory.{active_provider}.cli"
    try:
