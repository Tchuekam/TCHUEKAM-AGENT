...
Config file: `$TCHUEKAM_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `tchuekam` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `tchuekam-{identity}` → `tchuekam-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per TchuEkaM profile:

```json
{
  "container_tag": "tchuekam-{identity}"
}
```

For a profile named `coder`, this resolves to `tchuekam-coder`. The default profile resolves to `tchuekam-default`. Without `{identity}`, all profiles share the same container.
