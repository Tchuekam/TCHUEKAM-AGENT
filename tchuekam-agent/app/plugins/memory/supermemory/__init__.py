...
_DEFAULT_CONTAINER_TAG = "tchuekam"
...
def _load_supermemory_config(tchuekam_home: str) -> dict:
    config = _default_config()
    config_path = Path(tchuekam_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, tchuekam_home: str) -> None:
    config_path = Path(tchuekam_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `tchuekam memory setup`.
        # All other options are documented for $TCHUEKAM_HOME/supermemory.json
    ...
    def save_config(self, values, tchuekam_home):
        ...
        _save_supermemory_config(sanitized, tchuekam_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from tchuekam_constants import get_tchuekam_home
        self._tchuekam_home = kwargs.get("tchuekam_home") or str(get_tchuekam_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._tchuekam_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "tchuekam", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "tchuekam_memory", "target": target, "type": "explicit_memory"},
