...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/tchuekam>
...
```bash
tchuekam honcho setup    # full interactive wizard (cloud or local)
tchuekam memory setup    # generic picker, also works
```

Or manually:
```bash
tchuekam config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$TCHUEKAM_HOME/honcho.json` | Profile-local (isolated TchuEkaM instances) |
| 2 | `~/.tchuekam/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active TchuEkaM profile: `tchuekam` (default) or `tchuekam.<profile>`.
...
| 4 | `per-session` strategy | TchuEkaM session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`tchuekam-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`tchuekam`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-tchuekam-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening tchuekam in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — TchuEkaM session ID (timestamp + hex). Every `tchuekam` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple TchuEkaM profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`tchuekam`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active TchuEkaM profile: `tchuekam` (default) or `tchuekam.<profile>` (e.g. `tchuekam -p coder` → host key `tchuekam.coder`).
...
| `TCHUEKAM_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `tchuekam honcho setup` | Full interactive setup wizard |
| `tchuekam honcho status` | Show resolved config for active profile |
| `tchuekam honcho enable` / `disable` | Toggle Honcho for active profile |
| `tchuekam honcho mode <mode>` | Change recall or observation mode |
| `tchuekam honcho peer --user <name>` | Update user peer name |
| `tchuekam honcho peer --ai <name>` | Update AI peer name |
| `tchuekam honcho tokens --context <N>` | Set context token budget |
| `tchuekam honcho tokens --dialectic <N>` | Set dialectic max chars |
| `tchuekam honcho map <name>` | Map current directory to a session name |
| `tchuekam honcho sync` | Create host blocks for all TchuEkaM profiles |
