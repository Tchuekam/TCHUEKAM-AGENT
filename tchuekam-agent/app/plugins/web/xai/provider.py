...
``{title, url, description, position}`` rows every other TchuEkaM web
provider produces.
...
prefers TchuEkaM-managed xAI Grok OAuth (via ``tchuekam auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.tchuekam/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``tchuekam tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `tchuekam auth` to sign in with "
