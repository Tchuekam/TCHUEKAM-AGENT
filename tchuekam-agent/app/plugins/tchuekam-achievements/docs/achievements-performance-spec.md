# TchuEkaM Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: tchuekam-achievements plugin
...
### `GET /api/plugins/tchuekam-achievements/achievements`
...
### `POST /api/plugins/tchuekam-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/tchuekam-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/tchuekam-achievements/overview`
...
- Persist plugin data under:
  - `~/.tchuekam/plugins/tchuekam-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.tchuekam/plugins/tchuekam-achievements/scan_checkpoint.json`
...
- `~/.tchuekam/plugins/tchuekam-achievements/`
