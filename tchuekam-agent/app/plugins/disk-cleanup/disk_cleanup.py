"""disk_cleanup — ephemeral file cleanup for TchuEkaM Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under TCHUEKAM_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly TCHUEKAM_HOME and /tmp/tchuekam-*
Never touches: ~/.tchuekam/logs/ or any system directory.
"""
...
try:
    ...
    def get_tchuekam_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("TCHUEKAM_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".tchuekam").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$TCHUEKAM_HOME/logs/``."""
    return get_tchuekam_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$TCHUEKAM_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under TCHUEKAM_HOME or ``/tmp/tchuekam-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    tchuekam_home = get_tchuekam_home()
    try:
        path.resolve().relative_to(tchuekam_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/tchuekam-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("tchuekam-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside TCHUEKAM_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under TCHUEKAM_HOME (but leave TCHUEKAM_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "tchuekam-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(tchuekam_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == tchuekam_home:
                continue
            try:
                rel_parts = dirpath.relative_to(tchuekam_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    tchuekam_home = get_tchuekam_home()
    try:
        rel = path.resolve().relative_to(tchuekam_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "tchuekam-agent",
        }:
            return None
        ...
        # Path isn't under TCHUEKAM_HOME (e.g. /tmp/tchuekam-*) — fall through.
        pass
