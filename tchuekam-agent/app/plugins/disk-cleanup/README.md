...
Auto-tracks and cleans up ephemeral files created during TchuEkaM Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$TCHUEKAM_HOME` and `/tmp/tchuekam-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `TCHUEKAM_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `TCHUEKAM_HOME` or `/tmp/tchuekam-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$TCHUEKAM_HOME/disk-cleanup/` is itself excluded
- `$TCHUEKAM_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
