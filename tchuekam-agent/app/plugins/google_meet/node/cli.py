"""`tchuekam meet node ...` subcommand tree.

Wired into the existing ``tchuekam meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``tchuekam meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="tchuekam-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``tchuekam meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             tchuekam meet node approve <name> ws://<host>:{args.port} {token}")
        try:
