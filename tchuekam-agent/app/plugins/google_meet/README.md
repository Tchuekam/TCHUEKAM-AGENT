...
Let the tchuekam agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where tchuekam runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `tchuekam meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `tchuekam meet` CLI |
...
| `cli.py` | `tchuekam meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$TCHUEKAM_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `tchuekam meet node {run,list,approve,remove,status,ping}` |
...
tchuekam meet install                                      # pip + Chromium
tchuekam meet setup                                        # preflight
tchuekam meet auth                                         # optional
tchuekam meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
tchuekam meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.tchuekam/.env
tchuekam meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
tchuekam meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
tchuekam meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.tchuekam/.env
tchuekam meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, tchuekam will **not** switch your system audio input automatically — the
...
tchuekam plugins enable google_meet
tchuekam meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
tchuekam meet node ping my-mac
