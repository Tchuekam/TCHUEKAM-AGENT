...
session-scoped state file under ``$TCHUEKAM_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $TCHUEKAM_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_tchuekam_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this tchuekam install, leave it first —
    ...
    env = os.environ.copy()
    env["TCHUEKAM_MEET_URL"] = url
    env["TCHUEKAM_MEET_OUT_DIR"] = str(out)
    env["TCHUEKAM_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["TCHUEKAM_MEET_HEADED"] = "1"
    if auth_state:
        env["TCHUEKAM_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["TCHUEKAM_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if TCHUEKAM_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["TCHUEKAM_MEET_MODE"] = mode
    if realtime_model:
        env["TCHUEKAM_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["TCHUEKAM_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["TCHUEKAM_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["TCHUEKAM_MEET_REALTIME_KEY"] = realtime_api_key
