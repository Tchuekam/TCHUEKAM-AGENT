"""SimpleX Chat platform adapter (TchuEkaM plugin).
...
This adapter ships as a TchuEkaM platform plugin under
``plugins/platforms/simplex/``. The TchuEkaM plugin loader scans the
...
discoverable and `tchuekam setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside TchuEkaM — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"tchuekam-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``tchuekam setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.tchuekam/.env`` via ``tchuekam_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("tchuekam_cli.config not available; set SIMPLEX_* vars manually in ~/.tchuekam/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `tchuekam gateway status` without
