"""
Microsoft Teams platform adapter for TchuEkaM Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "TchuEkaM"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        tchuekam_action = data.get("tchuekam_action", "")
        session_key = data.get("session_key", "")

        if not tchuekam_action or not session_key:
        ...
        choice = choice_map.get(tchuekam_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="tchuekam_approve",
                    data={**btn_data_base, "tchuekam_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="tchuekam_approve",
                    data={**btn_data_base, "tchuekam_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="tchuekam_approve",
                    data={**btn_data_base, "tchuekam_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="tchuekam_approve",
                    data={**btn_data_base, "tchuekam_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"TchuEkaM\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.tchuekam/.env")
