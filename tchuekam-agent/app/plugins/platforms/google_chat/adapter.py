...
# ``model_tools`` import time, so ``tchuekam status``, ``tchuekam chat``, etc.
...
# ``tchuekam_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "TchuEkaM is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a tchuekam MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${TCHUEKAM_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _tchuekam_home = _get_tchuekam_home()
        except (ModuleNotFoundError, ImportError):
            _tchuekam_home = _Path.home() / ".tchuekam"
        self._thread_count_store = _ThreadCountStore(
            _tchuekam_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "TchuEkaM is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("TCHUEKAM_HOME", str(_Path.home() / ".tchuekam"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${TCHUEKAM_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a tchuekam MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'TchuEkaM is thinking…' marker message.
        ...
        "TchuEkaM is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "TchuEkaM is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "TchuEkaM is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "TchuEkaM is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "TchuEkaM is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``tchuekam setup``.

    The setup wizard at ``tchuekam_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.tchuekam/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. tchuekam-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.tchuekam/.env")
    print_info("Restart the gateway: tchuekam gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``tchuekam cron`` running as a
    separate process from ``tchuekam gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the TchuEkaM plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'tchuekam-agent[google_chat]'",
        ...
        # the "TchuEkaM is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'TchuEkaM is thinking…' marker message "
