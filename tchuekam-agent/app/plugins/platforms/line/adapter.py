"""
LINE Messaging API platform adapter for TchuEkaM Agent.
...
to TchuEkaM Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``TCHUEKAM_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            tchuekam_home = Path.home().joinpath(".tchuekam").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``tchuekam status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``tchuekam status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``tchuekam setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.tchuekam/.env`` via ``tchuekam_cli.config``.
    """
    print()
    ...
    try:
        from tchuekam_cli.config import get_env_var, set_env_var
    except ImportError:
        print("tchuekam_cli.config not available; set LINE_* vars manually in ~/.tchuekam/.env")
        return
