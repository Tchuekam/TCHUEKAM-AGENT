...
This page covers all commands related to [TchuEkaM profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `tchuekam profile` without a subcommand shows help.
...
## `tchuekam profile use`

```bash
tchuekam profile use <name>
```

Sets `<name>` as the active profile. All subsequent `tchuekam` commands (without `-p`) will use this profile.
...
tchuekam profile use default
```

## `tchuekam profile create`

```bash
tchuekam profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `tchuekam profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `tchuekam update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
tchuekam profile create mybot

# Clone config only from current profile
tchuekam profile create work --clone

# Clone everything from current profile
tchuekam profile create backup --clone-all

# Clone config from a specific profile
tchuekam profile create work2 --clone --clone-from work
```

## `tchuekam profile describe`

```bash
tchuekam profile describe [<name>] [options]
```
...
tchuekam profile describe researcher

# Set it explicitly
tchuekam profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
tchuekam profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
tchuekam profile describe --all --auto
```

## `tchuekam profile delete`

```bash
tchuekam profile delete <name> [options]
```
...
```bash
tchuekam profile delete mybot
tchuekam profile delete mybot --yes
```
...
## `tchuekam profile show`

```bash
tchuekam profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's TchuEkaM home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ tchuekam profile show work
Profile: work
Path:    ~/.tchuekam/profiles/work
...
## `tchuekam profile alias`

```bash
tchuekam profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your TchuEkaM installation.
...
```bash
tchuekam profile alias work
# Creates/updates ~/.local/bin/work

tchuekam profile alias work --name mywork
# Creates ~/.local/bin/mywork

tchuekam profile alias work --remove
# Removes the wrapper script
```

## `tchuekam profile rename`

```bash
tchuekam profile rename <old-name> <new-name>
```
...
```bash
tchuekam profile rename mybot assistant
# ~/.tchuekam/profiles/mybot → ~/.tchuekam/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `tchuekam profile export`

```bash
tchuekam profile export <name> [options]
```
...
```bash
tchuekam profile export work
# Creates work.tar.gz in the current directory

tchuekam profile export work -o ./work-2026-03-29.tar.gz
```

## `tchuekam profile import`

```bash
tchuekam profile import <archive> [options]
```
...
```bash
tchuekam profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

tchuekam profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`tchuekam profile export` / `import` are still the right commands for
...
### `tchuekam profile install`

```bash
tchuekam profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `tchuekam -p telemetry`). |
...
tchuekam profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
tchuekam profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
tchuekam profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
tchuekam profile install ./telemetry/
```

### `tchuekam profile update`

```bash
tchuekam profile update <name> [--force-config] [--yes]
```
...
### `tchuekam profile info`

```bash
tchuekam profile info <name>
```

Prints the profile's distribution manifest — name, version, required
TchuEkaM version, author, env var requirements, the source URL/path, and
...
`tchuekam profile list` also shows the distribution name and version in a
`Distribution` column, and `tchuekam profile show <name>` / `delete <name>`
...
tchuekam profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
tchuekam profile install https://github.com/your-org/internal-assistant.git
```
...
`tchuekam_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
TchuEkaM version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host TchuEkaM can clone from.
3. Tell recipients to run `tchuekam profile install <your-repo-url>`.
...
## `tchuekam -p` / `tchuekam --profile`

```bash
tchuekam -p <name> <command> [options]
tchuekam --profile <name> <command> [options]
```

Global flag to run any TchuEkaM command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
tchuekam -p work chat -q "Check the server status"
tchuekam --profile dev gateway start
tchuekam -p personal skills list
tchuekam -p work config edit
```

## `tchuekam completion`

```bash
tchuekam completion <shell>
```
...
- `tchuekam profile <TAB>` — subcommands (list, use, create, etc.)
- `tchuekam profile use <TAB>` — profile names
- `tchuekam -p <TAB>` — profile names
