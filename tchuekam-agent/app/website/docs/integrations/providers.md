...
This page covers setting up inference providers for TchuEkaM Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use TchuEkaM.

## Inference Providers

You need at least one way to connect to an LLM. Use `tchuekam model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `tchuekam model` (OAuth, subscription-based) |
| **OpenAI Codex** | `tchuekam model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `tchuekam model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `tchuekam model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `tchuekam model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.tchuekam/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.tchuekam/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.tchuekam/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.tchuekam/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.tchuekam/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.tchuekam/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.tchuekam/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.tchuekam/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.tchuekam/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.tchuekam/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.tchuekam/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `tchuekam model` → "xAI Grok OAuth (SuperGrok Subscription)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.tchuekam/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.tchuekam/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.tchuekam/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.tchuekam/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.tchuekam/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.tchuekam/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.tchuekam/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.tchuekam/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.tchuekam/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `tchuekam model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **LM Studio** | `tchuekam model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `tchuekam model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
[Nous Portal](https://portal.nousresearch.com) is Nous Research's unified subscription gateway and **the recommended way to run TchuEkaM Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/docs/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) plus [Nous Chat](https://chat.nousresearch.com) — billed against your Nous subscription instead of separate per-provider accounts.

```bash
tchuekam setup --portal     # fresh install — OAuth + provider + gateway in one command
tchuekam model              # existing install — pick "Nous Portal" from the list
tchuekam portal status      # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Nous Portal integration page](/docs/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run TchuEkaM Agent with Nous Portal guide](/docs/guides/run-tchuekam-with-nous-portal).


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). TchuEkaM stores the resulting credentials in its own auth store under `~/.tchuekam/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), TchuEkaM marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `tchuekam auth add codex-oauth` (or `tchuekam model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), TchuEkaM routes these tasks to your **main chat model** — the same model you picked in `tchuekam model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/docs/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/docs/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `tchuekam setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `tchuekam model` or per-tool from `tchuekam tools`. Inspect routing at any time with `tchuekam portal status`.
:::

### Two Commands for Model Management

TchuEkaM has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`tchuekam model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a TchuEkaM chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `tchuekam model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `tchuekam model`, complete the provider setup, then start a new session.
...
When you authenticate via `tchuekam model` → Anthropic OAuth (or via `tchuekam auth add anthropic --type oauth`), TchuEkaM routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by TchuEkaM — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
tchuekam chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `tchuekam model`
# TchuEkaM will use Claude Code's credential store directly when available
tchuekam model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
tchuekam chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
tchuekam chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `tchuekam model`, TchuEkaM prefers Claude Code's own credential store over copying the token into `~/.tchuekam/.env`. That keeps refreshable Claude credentials refreshable.
...
TchuEkaM supports GitHub Copilot as a first-class provider with two modes:
...
```bash
tchuekam chat --provider copilot --model gpt-5.4
```
...
If no token is found, `tchuekam model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `tchuekam model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `tchuekam model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in TchuEkaM
TchuEkaM sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, TchuEkaM now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). TchuEkaM therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
tchuekam chat --provider copilot-acp --model copilot-acp
...
| `TCHUEKAM_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `TCHUEKAM_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
tchuekam chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.tchuekam/.env

# z.ai / ZhipuAI GLM
tchuekam chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.tchuekam/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
tchuekam chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.tchuekam/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
tchuekam chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.tchuekam/.env

# MiniMax (global endpoint)
tchuekam chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.tchuekam/.env

# MiniMax (China endpoint)
tchuekam chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.tchuekam/.env

# Qwen Cloud / DashScope (Qwen models)
tchuekam chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.tchuekam/.env

# Xiaomi MiMo
tchuekam chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.tchuekam/.env

# Tencent TokenHub (Hy3 Preview)
tchuekam chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.tchuekam/.env

# Arcee AI (Trinity models)
tchuekam chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.tchuekam/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
tchuekam chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.tchuekam/.env
```
...
When using the Z.AI / GLM provider, TchuEkaM automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.tchuekam/.env` and pick xAI in `tchuekam model`, or drop `grok` as a shortcut into `/model grok-4-1-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok Subscription)** in `tchuekam model`, or run `tchuekam auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if TchuEkaM runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), TchuEkaM automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `tchuekam tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.
...
# Short alias
tchuekam chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `tchuekam model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and TchuEkaM auto-discovers the available models.
...
tchuekam chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 tchuekam chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `tchuekam model`, sign in through the browser, and TchuEkaM persists the refresh token.

```bash
tchuekam model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.tchuekam/auth.json

tchuekam chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `TCHUEKAM_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), TchuEkaM exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
tchuekam chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `tchuekam model`, sign in through the browser, and TchuEkaM persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
tchuekam chat   # uses api.minimax.io/anthropic endpoint
```
...
tchuekam chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.tchuekam/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 tchuekam chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
TchuEkaM automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
tchuekam chat --provider gmi --model deepseek-ai/DeepSeek-R1
# Requires: GMI_API_KEY in ~/.tchuekam/.env
```
...
```bash
# StepFun
tchuekam chat --provider stepfun --model step-3-mini
# Requires: STEPFUN_API_KEY in ~/.tchuekam/.env
```
...
```bash
# Use any available model
tchuekam chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.tchuekam/.env

# Short alias
tchuekam chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
# → done — TchuEkaM auto-provisions your free tier on first request
```

TchuEkaM ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.tchuekam/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `TCHUEKAM_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of TchuEkaM works unchanged
...
| Workspace / Standard / Enterprise account | Set `TCHUEKAM_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | TchuEkaM detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. TchuEkaM shows
...
```bash
TCHUEKAM_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
TCHUEKAM_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
TchuEkaM Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point TchuEkaM at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of TchuEkaM — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `tchuekam setup` or config migration. Use `tchuekam model` or edit `config.yaml` directly.
:::
...
:::warning tchuekam model vs /model
**`tchuekam model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active TchuEkaM chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `tchuekam model`, set up the new provider, then start a new session.
:::
...
When switching providers, TchuEkaM persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like TchuEkaM.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in TchuEkaM) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `tchuekam` (Qwen 2.5, TchuEkaM 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure TchuEkaM to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but TchuEkaM won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), TchuEkaM 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
TchuEkaM will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, TchuEkaM) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since TchuEkaM Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your TchuEkaM config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your TchuEkaM config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with TchuEkaM.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running TchuEkaM inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser tchuekam` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. TchuEkaM's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what TchuEkaM thinks the context is
...
TchuEkaM auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), TchuEkaM uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure TchuEkaM with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in TchuEkaM.
...
Then configure TchuEkaM with `tchuekam model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure TchuEkaM with `tchuekam model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `tchuekam model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). TchuEkaM uses this to decide when to compress history and to validate API requests.
...
TchuEkaM uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`tchuekam model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — TchuEkaM uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `tchuekam model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and TchuEkaM will merge it into each chat-completions request for that endpoint:
...
The `tchuekam model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `tchuekam model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.tchuekam/config.yaml` and the matching API key goes in `~/.tchuekam/.env`.
...
Together's `/v1/models` endpoint works, so `tchuekam model` can auto-discover available models.
...
:::tip Troubleshooting
- `tchuekam doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `tchuekam model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `tchuekam model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point TchuEkaM at your instance (no API key needed):
   ```bash
   tchuekam config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.tchuekam/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.tchuekam/config.yaml`:
...
Configure a chain of backup providers TchuEkaM tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `tchuekam fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/docs/user-guide/features/fallback-providers).
:::
