...
title: "Git Worktrees"
description: "Run multiple TchuEkaM agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

TchuEkaM Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with TchuEkaM so each session has a clean, isolated working directory.

## Why Use Worktrees with TchuEkaM?

TchuEkaM treats the **current working directory** as the project root:

- CLI: the directory where you run `tchuekam` or `tchuekam chat`
...
git worktree add ../repo-feature feature/tchuekam-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/tchuekam-experiment` checked out in that directory

Now you can `cd` into the new worktree and run TchuEkaM there:

```bash
cd ../repo-feature

# Start TchuEkaM in the worktree
tchuekam
```
...
git worktree add ../repo-experiment-a feature/tchuekam-a
git worktree add ../repo-experiment-b feature/tchuekam-b
```
...
Each TchuEkaM process:

- Works on its own branch (`feature/tchuekam-a` vs `feature/tchuekam-b`).
...
- TchuEkaM checkpoint data under `~/.tchuekam/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per TchuEkaM experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/tchuekam-checkpoints-docs`, `feature/tchuekam-refactor-tests`.
- **Commit frequently**
...
- **Avoid running TchuEkaM from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `tchuekam -w` (Automatic Worktree Mode)

TchuEkaM has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `tchuekam/tchuekam-<hash>`).
...
```bash
tchuekam -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `tchuekam -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each TchuEkaM session its own clean checkout.
