...
All skills live in **`~/.tchuekam/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point TchuEkaM at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling TchuEkaM to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.tchuekam/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
tchuekam chat --toolsets skills -q "What skills do you have?"
tchuekam chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.tchuekam/cache/chart-q4-2025.png
...
When a missing value is encountered, TchuEkaM asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `tchuekam setup` or `~/.tchuekam/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `tchuekam config migrate` prompts for unconfigured settings, and `tchuekam config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.tchuekam/skills/                  # Single source of truth
...
If you maintain skills outside of TchuEkaM — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell TchuEkaM to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.tchuekam/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.tchuekam/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the TchuEkaM process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, TchuEkaM ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.tchuekam/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.tchuekam/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `tchuekam bundles list`.
...
tchuekam bundles create research

# Overwrite an existing bundle
tchuekam bundles create backend-dev --skill ... --force

# Delete a bundle
tchuekam bundles delete backend-dev

# Re-scan ~/.tchuekam/skill-bundles/ and report changes
tchuekam bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.tchuekam/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.tchuekam/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
tchuekam skills browse                              # Browse all hub skills (official first)
tchuekam skills browse --source official            # Browse only official optional skills
tchuekam skills search kubernetes                   # Search all sources
tchuekam skills search react --source skills-sh     # Search the skills.sh directory
tchuekam skills search https://mintlify.com/docs --source well-known
tchuekam skills inspect openai/skills/k8s           # Preview before installing
tchuekam skills install openai/skills/k8s           # Install with security scan
tchuekam skills install official/security/1password
tchuekam skills install skills-sh/vercel-labs/json-render/json-render-react --force
tchuekam skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
tchuekam skills list --source hub                   # List hub-installed skills
tchuekam skills check                               # Check installed hub skills for upstream updates
tchuekam skills update                              # Reinstall hub skills with upstream changes when needed
tchuekam skills audit                               # Re-scan all hub skills for security
tchuekam skills uninstall k8s                       # Remove a hub skill
tchuekam skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
tchuekam skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
tchuekam skills publish skills/my-skill --to github --repo owner/repo
tchuekam skills snapshot export setup.json          # Export skill config
tchuekam skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with TchuEkaM. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `tchuekam skills search <query> --source skills-sh`. TchuEkaM resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
TchuEkaM currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the TchuEkaM repository itself and install with builtin trust.
...
This is Vercel's public skills directory. TchuEkaM can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
tchuekam skills search https://mintlify.com/docs --source well-known
tchuekam skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
tchuekam skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

TchuEkaM can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
tchuekam skills install openai/skills/k8s
tchuekam skills tap add myorg/skills-repo
```
...
- TchuEkaM source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

TchuEkaM supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

TchuEkaM source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

TchuEkaM can search and convert agent entries from LobeHub's public catalog into installable TchuEkaM skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- TchuEkaM source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

TchuEkaM integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with TchuEkaM' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- TchuEkaM source id: `browse-sh`
- Trust level: `community`

```bash
tchuekam skills search airbnb --source browse-sh
tchuekam skills inspect browse-sh/airbnb.com/search-listings-ddgioa
tchuekam skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). TchuEkaM fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- TchuEkaM source id: `url`
...
```bash
tchuekam skills install https://sharethis.chat/SKILL.md
tchuekam skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
tchuekam skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `tchuekam skills update` re-fetches from the same URL automatically when you want to refresh.
...
`tchuekam skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with TchuEkaM | Always trusted |
...
```bash
tchuekam skills check          # Report which installed hub skills changed upstream
tchuekam skills update         # Reinstall only the skills with updates available
tchuekam skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other TchuEkaM users add with `tchuekam skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.tchuekam.tags`, `version`, `author`, `platforms`, `metadata.tchuekam.config`).
...
TchuEkaM discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/tchuekam-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any TchuEkaM user can subscribe and install:

```bash
tchuekam skills tap add my-org/tchuekam-skills
tchuekam skills search deploy
tchuekam skills install my-org/tchuekam-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.tchuekam/.hub/taps.json`:
...
The `tchuekam skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `tchuekam skills tap list` shows the effective path per tap.
...
```bash
tchuekam skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.tchuekam/.hub/taps.json` (created on demand).

## Bundled skill updates (`tchuekam skills reset`)

TchuEkaM ships with a set of bundled skills in `skills/` inside the repo. On install and on every `tchuekam update`, a sync pass copies those into `~/.tchuekam/skills/` and records a manifest at `~/.tchuekam/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, TchuEkaM recomputes the hash of your local copy and compares it to the origin hash:
...
tchuekam skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `TCHUEKAM_HOME`, so `tchuekam -p coder skills reset <name>` only affects that profile.
:::
