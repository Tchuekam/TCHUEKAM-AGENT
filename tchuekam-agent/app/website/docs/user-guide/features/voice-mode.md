...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with TchuEkaM Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

TchuEkaM Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with TchuEkaM](/docs/guides/use-voice-mode-with-tchuekam).

## Prerequisites

Before using voice features, make sure you have:

1. **TchuEkaM Agent installed** — `pip install tchuekam-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `tchuekam model` or set your preferred provider credentials in `~/.tchuekam/.env`
3. **A working base setup** — run `tchuekam` to verify the agent responds to text before enabling voice

:::tip
The `~/.tchuekam/` directory and default `config.yaml` are created automatically the first time you run `tchuekam`. You only need to create `~/.tchuekam/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/docs/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `tchuekam setup --portal` wires both up at once.
:::
...
pip install "tchuekam-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "tchuekam-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "tchuekam-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "tchuekam-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`tchuekam chat`) and the **TUI** (`tchuekam --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.tchuekam/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
tchuekam                # Start the interactive CLI
```
...
1. Start the CLI with `tchuekam` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.tchuekam/config.yaml` (default: `ctrl+b`).
:::
...
```bash
tchuekam gateway        # Start the gateway (connects to configured platforms)
tchuekam gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@tchuekambyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.tchuekam/.env`:
```bash
...
```bash
tchuekam gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.tchuekam/.env`
...
- Review gateway logs: `tail -f ~/.tchuekam/logs/gateway.log`
