...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let TchuEkaM default to FLUX 2 Klein. |
...
```bash
tchuekam setup --portal     # Nous OAuth, set Nous as provider, and turn on the Tool Gateway in one go
```

Already have TchuEkaM configured? Just switch your provider:

```bash
tchuekam model              # Pick Nous Portal — TchuEkaM will offer to turn on the Tool Gateway
```

When you select Nous Portal, TchuEkaM offers to turn on the Tool Gateway. Accept, and you're done — every supported tool is live on the next run.

Check what's active at any time:

```bash
tchuekam portal status      # Portal auth + Tool Gateway routing summary
tchuekam portal tools       # Gateway catalog with current routing per tool
tchuekam status             # Full system status (Tool Gateway is one section)
```

`tchuekam portal status` shows a section like:
...
```bash
tchuekam tools          # Interactive picker for each tool category
```
...
The set evolves — `tchuekam tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `tchuekam model` and `tchuekam tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.
...
```yaml
web:
  use_gateway: false   # TchuEkaM now uses FIRECRAWL_API_KEY from .env
```

`tchuekam tools` automatically clears the flag when you pick a non-gateway provider, so this usually happens for you.

### Self-hosted gateway (advanced)

Running your own Nous-compatible gateway? Override endpoints in `~/.tchuekam/.env`:
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `tchuekam tools`. TchuEkaM shows a clear error pointing at the portal.
...
Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `tchuekam setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. When `use_gateway: true`, TchuEkaM skips direct keys and uses the gateway. Flip the flag back to `false` and your keys become the source again. The gateway isn't a lock-in.
