...
chat completions — use your TchuEkaM-managed provider subscription as their
...
| Use case | "Use TchuEkaM as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. TchuEkaM stores
the refresh token in `~/.tchuekam/auth.json` — the same place all TchuEkaM
provider logins live.

### 2. Start the proxy

```bash
tchuekam proxy start
```

```
Starting TchuEkaM proxy for Nous Portal
...
Model:      TchuEkaM-4-70B    # or TchuEkaM-4.3-36B, TchuEkaM-4-405B
```
...
```bash
tchuekam proxy providers
```

Currently shipped: `nous` (Nous Portal). More OAuth providers can be
added by implementing the `UpstreamAdapter` interface in
`tchuekam_cli/proxy/adapters/`.

## Check status

```bash
tchuekam proxy status
```

```
TchuEkaM proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `tchuekam login nous`. If you see
...
INFERENCE_TEXT_MODEL=TchuEkaM-4-70B
```
...
```bash
tchuekam proxy start --host 0.0.0.0 --port 8645
```
...
`tchuekam_cli/proxy/adapters/<provider>.py` and registering it in
