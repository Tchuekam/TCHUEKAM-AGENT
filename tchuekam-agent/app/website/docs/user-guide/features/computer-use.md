...
TchuEkaM Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
tchuekam computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `tchuekam computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `tchuekam tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
3. Grant macOS permissions when prompted:
   - **System Settings → Privacy & Security → Accessibility** → allow the
     terminal (or TchuEkaM app).
   - **System Settings → Privacy & Security → Screen Recording** → allow
     the same.
4. Start a session with the toolset enabled:
   ```
   tchuekam -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.tchuekam/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). TchuEkaM refreshes the binary in two
places so you don't get stuck on a stale release:

- **`tchuekam update`** — when you update TchuEkaM itself, if `cua-driver` is
...
- **`tchuekam computer-use install --upgrade`** — manual force-refresh.
...
`tchuekam computer-use status` shows the installed version next to the
binary path.
...
TchuEkaM applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.tchuekam/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. TchuEkaM applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `TCHUEKAM_CUA_DRIVER_VERSION`
...
```
TCHUEKAM_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
TCHUEKAM_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
TCHUEKAM_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`tchuekam computer-use install` to fetch the cua-driver binary, or run
`tchuekam tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/tchuekam-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
