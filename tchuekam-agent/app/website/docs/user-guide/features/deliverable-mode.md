...
When TchuEkaM Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.tchuekam/config.yaml` under `agent.custom_instructions`.
...
If you use TchuEkaM' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.tchuekam/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. TchuEkaM Agent's deliverable
