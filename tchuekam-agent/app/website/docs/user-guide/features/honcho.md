...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of TchuEkaM's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple TchuEkaM instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
tchuekam memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.tchuekam/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$TCHUEKAM_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `tchuekam` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — TchuEkaM syncs them back at session init.
...
The `tchuekam honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `tchuekam memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
tchuekam honcho status          # Connection status, config, and key settings
tchuekam honcho setup           # Redirects to `tchuekam memory setup`
tchuekam honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
tchuekam honcho peer            # Show or update peer names + dialectic reasoning level
tchuekam honcho mode            # Show or set recall mode (hybrid/context/tools)
tchuekam honcho tokens          # Show or set token budget for context and dialectic
tchuekam honcho identity        # Seed or show the AI peer's Honcho identity
tchuekam honcho sync            # Sync Honcho config to all existing profiles
tchuekam honcho peers           # Show peer identities across all profiles
tchuekam honcho sessions        # List known Honcho session mappings
tchuekam honcho map             # Map current directory to a Honcho session name
tchuekam honcho enable          # Enable Honcho for the active profile
tchuekam honcho disable         # Disable Honcho for the active profile
tchuekam honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `tchuekam honcho`

If you previously used the standalone `tchuekam honcho setup`:
...
No re-login or re-setup needed. Run `tchuekam memory setup` and select "honcho" — the wizard detects your existing config.
