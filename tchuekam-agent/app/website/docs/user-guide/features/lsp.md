...
TchuEkaM runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. TchuEkaM
...
1. TchuEkaM captures a baseline of current diagnostics for the file.
...
…). TchuEkaM auto-detects the binary on PATH or in
`<TCHUEKAM_HOME>/lsp/bin/`.
...
`node_modules` tree — TchuEkaM installs both packages together when you
run `tchuekam lsp install typescript` or auto-install fires on first
use.

## CLI

```
tchuekam lsp status          # service state + per-server install status
tchuekam lsp list            # registry, optionally --installed-only
tchuekam lsp install <id>    # eagerly install one server
tchuekam lsp install-all     # try every server with a known recipe
tchuekam lsp restart         # tear down running clients
tchuekam lsp which <id>      # print resolved binary path
```

`tchuekam lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <TCHUEKAM_HOME>/lsp/bin
...
When `install_strategy: auto`, TchuEkaM installs binaries into
`<TCHUEKAM_HOME>/lsp/bin/`. NPM packages land in
`<TCHUEKAM_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully TchuEkaM-owned and is
...
Servers are kept alive for the life of the TchuEkaM process. There's
...
**`tchuekam lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<TCHUEKAM_HOME>/lsp/bin/`. Run
`tchuekam lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `tchuekam lsp status`**
...
When `tchuekam lsp status` shows a `Backend warnings` section, install
...
`~/.tchuekam/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.tchuekam/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `tchuekam lsp restart` to clear the set;
