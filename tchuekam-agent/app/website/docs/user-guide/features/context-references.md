...
Type `@` followed by a reference to inject content directly into your message. TchuEkaM expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- TchuEkaM env: `$TCHUEKAM_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$TCHUEKAM_HOME/skills/.hub/`
