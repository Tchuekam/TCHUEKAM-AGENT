...
description: "Chat with TchuEkaM from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with TchuEkaM from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with TchuEkaM](/docs/guides/use-voice-mode-with-tchuekam).
...
```mermaid
flowchart TB
    subgraph Gateway["TchuEkaM Gateway"]
...
```bash
tchuekam gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
tchuekam gateway              # Run in foreground
tchuekam gateway setup        # Configure messaging platforms interactively
tchuekam gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo tchuekam gateway install --system   # Linux only: install a boot-time system service
tchuekam gateway start        # Start the default service
tchuekam gateway stop         # Stop the default service
tchuekam gateway status       # Check default service status
tchuekam gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update TchuEkaM Agent to the latest version |
...
Configure per-platform overrides in `~/.tchuekam/gateway.json`:
...
tchuekam pairing approve telegram XKGH5N7P

# Other pairing commands:
tchuekam pairing list          # View pending + approved users
tchuekam pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, TchuEkaM appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.tchuekam/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.tchuekam/config.yaml`:
...
```bash
TCHUEKAM_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
tchuekam gateway install               # Install as user service
tchuekam gateway start                 # Start the service
tchuekam gateway stop                  # Stop the service
tchuekam gateway status                # Check status
journalctl --user -u tchuekam-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo tchuekam gateway install --system
sudo tchuekam gateway start --system
sudo tchuekam gateway status --system
journalctl -u tchuekam-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. TchuEkaM will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple TchuEkaM installations on the same machine (with different `TCHUEKAM_HOME` directories), each gets its own systemd service name. The default `~/.tchuekam` uses `tchuekam-gateway`; other installations use `tchuekam-gateway-<hash>`. The `tchuekam gateway` commands automatically target the correct service for your current `TCHUEKAM_HOME`.
:::

### macOS (launchd)

```bash
tchuekam gateway install               # Install as launchd agent
tchuekam gateway start                 # Start the service
tchuekam gateway stop                  # Stop the service
tchuekam gateway status                # Check status
tail -f ~/.tchuekam/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.tchuekam.gateway.plist`. It includes three environment variables:
...
- **TCHUEKAM_HOME** — scopes the gateway to your TchuEkaM installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `tchuekam gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `TCHUEKAM_HOME` directory gets its own launchd label. The default `~/.tchuekam` uses `ai.tchuekam.gateway`; other installations use `ai.tchuekam.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `tchuekam-cli` | Full access |
| Telegram | `tchuekam-telegram` | Full tools including terminal |
| Discord | `tchuekam-discord` | Full tools including terminal |
| WhatsApp | `tchuekam-whatsapp` | Full tools including terminal |
| Slack | `tchuekam-slack` | Full tools including terminal |
| Google Chat | `tchuekam-google_chat` | Full tools including terminal |
| Signal | `tchuekam-signal` | Full tools including terminal |
| SMS | `tchuekam-sms` | Full tools including terminal |
| Email | `tchuekam-email` | Full tools including terminal |
| Home Assistant | `tchuekam-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `tchuekam-mattermost` | Full tools including terminal |
| Matrix | `tchuekam-matrix` | Full tools including terminal |
| DingTalk | `tchuekam-dingtalk` | Full tools including terminal |
| Feishu/Lark | `tchuekam-feishu` | Full tools including terminal |
| WeCom | `tchuekam-wecom` | Full tools including terminal |
| WeCom Callback | `tchuekam-wecom-callback` | Full tools including terminal |
| Weixin | `tchuekam-weixin` | Full tools including terminal |
| BlueBubbles | `tchuekam-bluebubbles` | Full tools including terminal |
| QQBot | `tchuekam-qqbot` | Full tools including terminal |
| Yuanbao | `tchuekam-yuanbao` | Full tools including terminal |
| Microsoft Teams | `tchuekam-teams` | Full tools including terminal |
| API Server | `tchuekam-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `tchuekam-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.tchuekam/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
