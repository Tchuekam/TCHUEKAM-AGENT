...
Connect TchuEkaM to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
TchuEkaM supports two WeCom integration modes:
...
3. TchuEkaM decrypts the message, queues it for the agent
...
(Use `tchuekam gateway start` only after `tchuekam gateway install` has registered the systemd/launchd service.)
