...
sidebar_position: 16
title: "Yuanbao"
description: "Connect TchuEkaM Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect TchuEkaM to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.tchuekam/.env`:
...
All standard TchuEkaM commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.tchuekam/logs/gateway.log`
...
4. Consider enabling verbose logging: `TCHUEKAM_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. TchuEkaM automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
TCHUEKAM_LOG_LEVEL=debug tchuekam gateway
```
...
```bash
tchuekam chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
