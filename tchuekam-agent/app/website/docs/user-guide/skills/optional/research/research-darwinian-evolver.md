...
| | |
|---|---|
| Source | Optional — install with `tchuekam skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), TchuEkaM Agent |
| License | MIT |
...
import upstream classes into TchuEkaM itself.
...
```bash
mkdir -p ~/.tchuekam/cache/darwinian-evolver && cd ~/.tchuekam/cache/darwinian-evolver
...
```bash
cd ~/.tchuekam/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.tchuekam/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.tchuekam/skills/research/darwinian-evolver
DE_DIR=~/.tchuekam/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside TchuEkaM core.
   Custom driver scripts under `~/.tchuekam/skills/...` are user-side and fine.
