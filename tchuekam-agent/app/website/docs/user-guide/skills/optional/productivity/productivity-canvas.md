...
| | |
|---|---|
| Source | Optional — install with `tchuekam skills install official/productivity/canvas` |
...
4. Name the token (e.g., "TchuEkaM Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.tchuekam/.env`:
...
```bash
CANVAS="python $TCHUEKAM_HOME/skills/productivity/canvas/scripts/canvas_api.py"
