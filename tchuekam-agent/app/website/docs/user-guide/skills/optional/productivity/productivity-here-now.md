...
| | |
|---|---|
| Source | Optional — install with `tchuekam skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${TCHUEKAM_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${TCHUEKAM_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${TCHUEKAM_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client tchuekam
```
...
```bash
PUBLISH="${TCHUEKAM_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client tchuekam
```
...
```bash
DRIVE="${TCHUEKAM_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `tchuekam`)    |
