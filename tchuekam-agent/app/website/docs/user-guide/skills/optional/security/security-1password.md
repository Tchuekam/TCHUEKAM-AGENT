...
| | |
|---|---|
| Source | Optional — install with `tchuekam skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by TchuEkaM Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during TchuEkaM terminal calls (desktop app flow only)
...
### Service Account (recommended for TchuEkaM)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.tchuekam/.env` (the skill will prompt for this on first load).
...
## TchuEkaM Execution Pattern (desktop app flow)

TchuEkaM terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/tchuekam-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/tchuekam-op.sock"
