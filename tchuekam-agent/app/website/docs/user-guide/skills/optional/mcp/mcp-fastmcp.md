...
| | |
|---|---|
| Source | Optional — install with `tchuekam skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into TchuEkaM or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to TchuEkaM. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.tchuekam/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.tchuekam/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is TchuEkaM integration, either:

- configure the server in `~/.tchuekam/config.yaml` using the `native-mcp` skill, or
...
### TchuEkaM cannot see the deployed server

The server-building part may be correct while the TchuEkaM config is not. Load the `native-mcp` skill and configure the server in `~/.tchuekam/config.yaml`, then restart TchuEkaM.
