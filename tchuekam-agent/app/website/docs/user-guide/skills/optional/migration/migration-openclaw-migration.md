---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into TchuEkaM Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into TchuEkaM Agent"
---
...
Migrate a user's OpenClaw customization footprint into TchuEkaM Agent. Imports TchuEkaM-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `tchuekam skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | TchuEkaM Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `TchuEkaM`, `Memory`, `Persona`, `Import` |
| Related skills | [`tchuekam-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-tchuekam-agent) |
...
# OpenClaw -> TchuEkaM Migration

Use this skill when a user wants to move their OpenClaw setup into TchuEkaM Agent with minimal manual cleanup.
...
```bash
tchuekam claw migrate              # Full interactive migration
...
tchuekam claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `tchuekam setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_tchuekam.py` to:

- import `SOUL.md` into the TchuEkaM home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into TchuEkaM memory entries
- merge OpenClaw command approval patterns into TchuEkaM `command_allowlist`
- migrate TchuEkaM-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.tchuekam/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen TchuEkaM workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.tchuekam/tts/`
- archive non-secret docs that do not have a direct TchuEkaM destination
...
- `scripts/openclaw_to_tchuekam.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.tchuekam/skills/migration/openclaw-migration/scripts/openclaw_to_tchuekam.py`

Do not guess a shorter path like `~/.tchuekam/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.tchuekam/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of TchuEkaM-compatible secrets, currently:
...
TchuEkaM CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in TchuEkaM. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a TchuEkaM workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing TchuEkaM skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.tchuekam/skills/migration/openclaw-migration/scripts/openclaw_to_tchuekam.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.tchuekam/skills/migration/openclaw-migration/scripts/openclaw_to_tchuekam.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.tchuekam/skills/migration/openclaw-migration/scripts/openclaw_to_tchuekam.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.tchuekam/skills/migration/openclaw-migration/scripts/openclaw_to_tchuekam.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.tchuekam/skills/migration/openclaw-migration/scripts/openclaw_to_tchuekam.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.tchuekam/skills/migration/openclaw-migration/scripts/openclaw_to_tchuekam.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of TchuEkaM unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty TchuEkaM targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean TchuEkaM destination. Unsupported auth blobs must still be reported as skipped.
...
- TchuEkaM persona state imported
- TchuEkaM memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.tchuekam/skills/openclaw-imports/`
