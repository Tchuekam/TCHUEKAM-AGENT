...
description: "Configure and use Honcho memory with TchuEkaM -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with TchuEkaM -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `tchuekam skills install official/autonomous-ai-agents/honcho` |
| Path | `optional-skills/autonomous-ai-agents/honcho` |
| Version | `2.0.0` |
| Author | TchuEkaM Agent |
| License | MIT |
...
# Honcho Memory for TchuEkaM

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every TchuEkaM profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/tchuekam#running-honcho-locally-with-tchuekam

### Verify

```bash
tchuekam honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this TchuEkaM profile's AI peer
...
Honcho models conversations as interactions between **peers**. TchuEkaM creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this TchuEkaM instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each TchuEkaM run |
| `global` | Single session across all directories |

Manual override: `tchuekam honcho map my-project-name`
...
Each TchuEkaM profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block tchuekam.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `tchuekam.coder` host block in `honcho.json`
...
```bash
tchuekam honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for TchuEkaM when Honcho memory is active.
...
Config file: `$TCHUEKAM_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `tchuekam honcho setup`. Ensure `memory.provider: honcho` is in `~/.tchuekam/config.yaml`.

### Memory not persisting across sessions
Check `tchuekam honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `tchuekam profile create <name> --clone`. For existing profiles: `tchuekam honcho sync`.
...
| Command | Description |
|---------|-------------|
| `tchuekam honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `tchuekam honcho status` | Show resolved config, connection test, peer info for active profile |
| `tchuekam honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `tchuekam honcho disable` | Disable Honcho for the active profile |
| `tchuekam honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `tchuekam honcho peers` | Show peer identities across all profiles |
| `tchuekam honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `tchuekam honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `tchuekam honcho sessions` | List known directory-to-session-name mappings |
| `tchuekam honcho map <name>` | Map current working directory to a Honcho session name |
| `tchuekam honcho identity` | Seed AI peer identity or show both peer representations |
| `tchuekam honcho sync` | Create host blocks for all TchuEkaM profiles that don't have one yet |
| `tchuekam honcho migrate` | Step-by-step migration guide from OpenClaw native memory to TchuEkaM + Honcho |
| `tchuekam memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `tchuekam memory status` | Show active memory provider and config |
| `tchuekam memory off` | Disable external memory provider |
