...
- Give it a name like "tchuekam-agent"
- Select scopes:
...
- Give it a title like "tchuekam-agent-&lt;machine-name>"
...
elif [ -f ~/.tchuekam/.env ] && grep -q "^GITHUB_TOKEN=" ~/.tchuekam/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.tchuekam/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
