...
```bash
tchuekam teams-pipeline validate
```
...
```bash
tchuekam teams-pipeline token-health
tchuekam teams-pipeline token-health --force-refresh
```
...
```bash
tchuekam teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
tchuekam teams-pipeline maintain-subscriptions
tchuekam teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: TchuEkaM cron (recommended if you already run the TchuEkaM gateway)

TchuEkaM ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.tchuekam/scripts/`. First create the script:

```bash
mkdir -p ~/.tchuekam/scripts
cat > ~/.tchuekam/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec tchuekam teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.tchuekam/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
tchuekam cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/tchuekam-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=TchuEkaM Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=tchuekam
EnvironmentFile=/etc/tchuekam/env
ExecStart=/usr/local/bin/tchuekam teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/tchuekam-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run TchuEkaM Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now tchuekam-teams-pipeline-maintain.timer
systemctl list-timers tchuekam-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/tchuekam teams-pipeline maintain-subscriptions >> /var/log/tchuekam/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.tchuekam/.env` at the top of a wrapper script that crontab calls.
...
```bash
tchuekam teams-pipeline subscriptions   # should show expirationDateTime advanced
tchuekam teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
tchuekam teams-pipeline list
tchuekam teams-pipeline list --status failed
tchuekam teams-pipeline show <job-id>
```

### Replay a stored job

```bash
tchuekam teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
tchuekam teams-pipeline fetch --meeting-id <meeting-id>
tchuekam teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `tchuekam teams-pipeline maintain-subscriptions --dry-run`
- inspect `tchuekam teams-pipeline list --status failed`
...
- run `tchuekam teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `tchuekam teams-pipeline run`
...
- [ ] `tchuekam teams-pipeline validate` returns an OK snapshot
- [ ] `tchuekam teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (TchuEkaM cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
