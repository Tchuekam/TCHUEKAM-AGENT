...
description: "Diagnose and fix common TchuEkaM cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`tchuekam gateway` for foreground, or `tchuekam gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `tchuekam cron tick`.
...
```bash
date
tchuekam cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.tchuekam/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.tchuekam/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.tchuekam/.env` |
...
| `local` | Write access to `~/.tchuekam/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `tchuekam cron list` for updated `last_error` field (if available).
...
```bash
tchuekam skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `tchuekam skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `tchuekam skills list`.
...
2. `~/.tchuekam/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `tchuekam cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the TchuEkaM config directory). Verify:
```bash
ls ~/.tchuekam/scripts/your-script.py   # Must exist
tchuekam cron edit <job_id> --script ~/.tchuekam/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `tchuekam skills install <skill-name>`.
...
```bash
ps aux | grep tchuekam
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.tchuekam/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.tchuekam/cron/jobs.json
chmod 600 ~/.tchuekam/cron/jobs.json   # Your user should own it
```
...
```bash
tchuekam cron list                    # Show all jobs, states, next_run times
tchuekam cron run <job_id>            # Schedule for next tick (for testing)
tchuekam cron edit <job_id>           # Fix configuration issues
tchuekam logs                         # View recent TchuEkaM logs
tchuekam skills list                  # Verify installed skills
```
...
1. Run the job with `tchuekam cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.tchuekam/logs/agent.log` for scheduler messages and `~/.tchuekam/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/tchuekam-agent](https://github.com/NousResearch/tchuekam-agent) with:
