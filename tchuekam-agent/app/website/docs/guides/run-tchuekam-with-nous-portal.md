...
3. Sets `model.provider: nous` in `~/.tchuekam/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where TchuEkaM is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
tchuekam setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
tchuekam auth add nous --type oauth --manual-paste
# Then re-run `tchuekam setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "TchuEkaM Agent release notes" and summarize the top 3 hits.
```

You should see TchuEkaM call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

The default after `tchuekam setup --portal` is a sensible general-purpose model, but the whole point of the subscription is access to the full catalog. Switch with `/model` mid-session:
...
tchuekam config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick TchuEkaM-4 for agent work

TchuEkaM-4-70B and TchuEkaM-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/docs/user-guide/features/subscription-proxy) from non-agent tools. For TchuEkaM Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a TchuEkaM-side opinion.
...
```bash
tchuekam setup voice
# → pick "Nous Subscription" for TTS
# → pick a speech-to-text backend (local faster-whisper is free, no setup)
```

Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and TchuEkaM will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
tchuekam cron add "Daily AI news summary" "every day at 9am" \
...
If you use [TchuEkaM profiles](/docs/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.tchuekam/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `tchuekam portal status` shows "not logged in" after `tchuekam setup --portal`
...
```bash
tchuekam config set model.provider nous
```
...
Re-verify with `tchuekam portal status`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so TchuEkaM doesn't replay it endlessly. Just log in again:

```bash
tchuekam auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/tchuekam-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`tchuekam portal status` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple TchuEkaM profiles where you're using the wrong one (check `tchuekam profile current`)

### Want to revoke and start clean

```bash
tchuekam auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/docs/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-TchuEkaM tools
...
- **[Profiles](/docs/user-guide/profiles)** — Share one Portal login across multiple TchuEkaM configurations
