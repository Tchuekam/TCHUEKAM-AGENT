...
This tutorial walks you through setting up a Telegram bot powered by TchuEkaM Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **TchuEkaM Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.tchuekam/.env`

:::tip
A $5/month VPS is plenty for running the gateway. TchuEkaM itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team TchuEkaM Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_tchuekam_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by TchuEkaM Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.tchuekam/.env`:
...
```
[Gateway] Starting TchuEkaM Gateway...
...
# View live logs
journalctl --user -u tchuekam-gateway -f
...
tchuekam gateway start
tchuekam gateway stop
tail -f ~/.tchuekam/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `tchuekam gateway install` to update the plist.
:::
...
```bash
tchuekam gateway stop && tchuekam gateway start
```
...
tchuekam pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.tchuekam/.env`:
...
Control how much detail the bot shows when using tools. In `~/.tchuekam/config.yaml`:
...
Customize how the bot communicates by editing `~/.tchuekam/SOUL.md`:

For a full guide, see [Use SOUL.md with TchuEkaM](/docs/guides/use-soul-with-tchuekam).
...
```bash
# From the CLI
tchuekam cron list          # View all scheduled jobs
tchuekam cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u tchuekam-gateway` (Linux) or `~/.tchuekam/logs/gateway.log` (macOS) |
| Cron job output | `~/.tchuekam/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.tchuekam/cron/jobs.json` |
| Pairing data | `~/.tchuekam/pairing/` |
| Session history | `~/.tchuekam/sessions/` |
