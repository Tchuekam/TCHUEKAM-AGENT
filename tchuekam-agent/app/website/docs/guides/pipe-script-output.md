...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `tchuekam send`."
---

# Pipe Script Output to Messaging Platforms

`tchuekam send` is a small, scriptable CLI that pushes a message to any
messaging platform TchuEkaM is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | tchuekam send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `tchuekam
...
tchuekam send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | tchuekam send --to telegram:-1001234567890

# Send a file
tchuekam send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
tchuekam send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
tchuekam send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
tchuekam send --list

# Filter by platform
tchuekam send --list telegram
```
...
Any platform TchuEkaM ships adapters for works as a target:
...
`tchuekam send` resolves the message body in this order:

1. **Positional argument** — `tchuekam send --to telegram "hi"`
2. **`--file PATH`** — `tchuekam send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | tchuekam send --to telegram`

When stdin is a TTY (no pipe), TchuEkaM does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  tchuekam send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `tchuekam send` reuses your TchuEkaM config, the same script works on
any host where TchuEkaM is installed — no need to export bot tokens into
...
`tchuekam send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  tchuekam send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | tchuekam send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.tchuekam/bin/tchuekam send \
...
```bash
./train.py --epochs 200 && \
  tchuekam send --to telegram "training done" || \
  tchuekam send --to telegram "training failed (exit $?)"
```
...
tchuekam send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(tchuekam send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `tchuekam send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `tchuekam send` calls
the platform's REST endpoint directly using credentials from
`~/.tchuekam/.env` and `~/.tchuekam/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `tchuekam gateway start` and retry.
...
# Machine-readable
tchuekam send --list --json
```

The listing is built from `~/.tchuekam/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`tchuekam gateway
...
| Approach | Multi-platform | Reuses TchuEkaM creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `tchuekam send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`tchuekam send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `tchuekam send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `tchuekam send` shares with cron delivery.
