...
description: "How TchuEkaM stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `tchuekam_cli/cron.py` | CLI `tchuekam cron` subcommands |
...
Jobs are stored in `~/.tchuekam/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `tchuekam cron` commands are run or during active CLI sessions.
...
```python
# ~/.tchuekam/scripts/check_competitors.py
...
2. **Environment variable** — `TCHUEKAM_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.tchuekam/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `tchuekam cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `tchuekam cron` CLI provides direct job management:

```bash
tchuekam cron list                    # Show all jobs
tchuekam cron create                  # Interactive job creation (alias: add)
tchuekam cron edit <job_id>           # Edit job configuration
tchuekam cron pause <job_id>          # Pause a running job
tchuekam cron resume <job_id>         # Resume a paused job
tchuekam cron run <job_id>            # Trigger immediate execution
tchuekam cron remove <job_id>         # Delete a job
```
