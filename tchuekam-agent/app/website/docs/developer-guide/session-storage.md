...
TchuEkaM Agent uses a SQLite database (`~/.tchuekam/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `tchuekam_state.py`


## Architecture Overview

```
~/.tchuekam/state.db (SQLite, WAL mode)
...
Multiple tchuekam processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.tchuekam/state.db
...
Default path: `~/.tchuekam/state.db`

This is derived from `tchuekam_constants.get_tchuekam_home()` which resolves to
`~/.tchuekam/` by default, or the value of `TCHUEKAM_HOME` environment variable.
