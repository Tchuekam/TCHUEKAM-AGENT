...
The ACP adapter wraps TchuEkaM' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
tchuekam acp / tchuekam-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.tchuekam/.env
  -> configure stderr logging
  -> construct TchuEkaMACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'tchuekam-agent[acp]==<version>' tchuekam-acp`, pointed at the `tchuekam-agent` PyPI release.
...
- `allow_once` -> TchuEkaM `once`
- `allow_always` -> TchuEkaM `always`
- reject options -> TchuEkaM `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps TchuEkaM tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["tchuekam-acp"])
...
Instead it reuses TchuEkaM' runtime resolver:

- `acp_adapter/auth.py`
- `tchuekam_cli/runtime_provider.py`

So ACP advertises and uses the currently configured TchuEkaM provider/credentials. It also always advertises a terminal setup auth method (`tchuekam-setup`, args `--setup`) so first-run registry clients can open TchuEkaM' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.tchuekam/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `tchuekam-acp` toolset definition
- `tchuekam_cli/main.py` — `tchuekam acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `tchuekam-acp` script
