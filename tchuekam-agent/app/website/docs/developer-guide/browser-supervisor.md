...
[PR #12550](https://github.com/NousResearch/tchuekam-agent/pull/12550) proposed a
...
(`tchuekam-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per TchuEkaM `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `tchuekam-acp`, `tchuekam-api-server`, core toolsets (gated on CDP reachability)
...
- `tchuekam_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
