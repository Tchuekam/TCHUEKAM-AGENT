...
description: "Run TchuEkaM Agent directly on an Android phone with Termux"
---

# TchuEkaM on Android with Termux

This is the tested path for running TchuEkaM Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop TchuEkaM from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

TchuEkaM now ships a Termux-aware installer path:
...
- links `tchuekam` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `tchuekam` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/tchuekam" "$PREFIX/bin/tchuekam"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `tchuekam` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.tchuekam/.env`.
...
### `tchuekam doctor` says ripgrep or Node is missing
