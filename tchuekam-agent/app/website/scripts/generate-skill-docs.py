...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/tchuekam-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    tchuekam_meta = (fm.get("metadata") or {}).get("tchuekam") or {}
    tags = tchuekam_meta.get("tags") or []
    related = tchuekam_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `tchuekam skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with TchuEkaM Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "TchuEkaM ships with a large built-in skill library copied into `~/.tchuekam/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "TchuEkaM also syncs bundled skills on `tchuekam update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.tchuekam/skills/` tree, it is still shipped with TchuEkaM; restore it with `tchuekam skills reset <name> --restore`.",
        "",
