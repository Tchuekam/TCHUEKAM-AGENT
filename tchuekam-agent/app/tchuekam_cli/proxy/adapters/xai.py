...
# xAI's public API is OpenAI-compatible for the endpoints TchuEkaM commonly
# uses. The Responses endpoint is included because TchuEkaM' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via TchuEkaM-managed OAuth credentials."""

    auth_hint = "tchuekam auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`tchuekam auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`tchuekam auth reset xai-oauth` or re-authenticate with "
                    "`tchuekam auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `tchuekam auth add xai-oauth --type oauth`."
            )
