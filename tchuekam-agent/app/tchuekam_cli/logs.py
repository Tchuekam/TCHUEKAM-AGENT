"""``tchuekam logs`` — view and filter TchuEkaM log files.
...
Usage examples::
    ...
    tchuekam logs                    # last 50 lines of agent.log
    tchuekam logs -f                 # follow agent.log in real time
    tchuekam logs errors             # last 50 lines of errors.log
    tchuekam logs gateway -n 100    # last 100 lines of gateway.log
    tchuekam logs --level WARNING    # only WARNING+ lines
    tchuekam logs --session abc123   # filter by session ID substring
    tchuekam logs --component tools  # only tool-related lines
    tchuekam logs --since 1h         # lines from the last hour
    tchuekam logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_tchuekam_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when TchuEkaM runs — try 'tchuekam chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_tchuekam_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_tchuekam_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_tchuekam_home()}/logs/")
        return

    print(f"Log files in {display_tchuekam_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'tchuekam chat' to generate logs)")
