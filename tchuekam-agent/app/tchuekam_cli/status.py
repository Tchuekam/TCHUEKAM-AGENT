"""
Status command for tchuekam CLI.

Shows the status of all TchuEkaM Agent components.
"""
...
from tchuekam_cli.auth import AuthError, resolve_provider
from tchuekam_cli.colors import Colors, color
from tchuekam_cli.config import get_env_path, get_env_value, get_tchuekam_home, load_config
from tchuekam_cli.models import provider_label
from tchuekam_cli.nous_subscription import get_nous_subscription_features
from tchuekam_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``tchuekam config``'s
...
def show_status(args):
    """Show status of all TchuEkaM Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ TchuEkaM Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: tchuekam auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: tchuekam model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: tchuekam auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: tchuekam auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: tchuekam model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from tchuekam_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'tchuekam-agent[vercel]')"
    ...
    try:
        from tchuekam_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   tchuekam gateway")
    ...
    jobs_file = get_tchuekam_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_tchuekam_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'tchuekam doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'tchuekam setup' to configure", Colors.DIM))
    print()
