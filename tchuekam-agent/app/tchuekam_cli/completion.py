"""Shell completion script generation for tchuekam CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_tchuekam_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# TchuEkaM Agent bash completion
# Add to ~/.bashrc:
#   eval "$(tchuekam completion bash)"

_tchuekam_profiles() {{
    local profiles_dir="$HOME/.tchuekam/profiles"
...
_tchuekam_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_tchuekam_profiles)" -- "$cur"))
        return
    fi
...
complete -F _tchuekam_completion tchuekam
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef tchuekam
# TchuEkaM Agent zsh completion
# Add to ~/.zshrc:
...
_tchuekam_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.tchuekam/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.tchuekam/profiles 2>/dev/null)}}")
    fi
...
_tchuekam() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_tchuekam_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'tchuekam command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# TchuEkaM Agent fish completion",
        "# Add to your config:",
        "#   tchuekam completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __tchuekam_profiles",
        "    echo default",
        "    if test -d $HOME/.tchuekam/profiles",
        "        ls $HOME/.tchuekam/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c tchuekam -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c tchuekam -f -s p -l profile"
        " -d 'Profile name' -xa '(__tchuekam_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__tchuekam_profiles)' -d 'Profile name'"
                )
