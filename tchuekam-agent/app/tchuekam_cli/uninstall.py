"""
TchuEkaM Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.tchuekam/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove TchuEkaM PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing tchuekam-agent or tchuekam PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# TchuEkaM Agent" comment and following line
                if '# TchuEkaM Agent' in line or '# tchuekam-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('tchuekam' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing tchuekam
                if 'tchuekam' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the tchuekam wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "tchuekam",
        Path("/usr/local/bin/tchuekam"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains tchuekam_cli reference)
                content = wrapper.read_text()
                if 'tchuekam_cli' in content or 'tchuekam-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``tchuekam gateway run`` processes
    ...
    try:
        from tchuekam_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from tchuekam_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``TCHUEKAM_HOME`` and ``TCHUEKAM_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\tchuekam\git\cmd``, ``%LOCALAPPDATA%\tchuekam\git\bin``,
#      ``%LOCALAPPDATA%\tchuekam\git\usr\bin``, ``%LOCALAPPDATA%\tchuekam\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\tchuekam\git\`` and Node to
#      ``%LOCALAPPDATA%\tchuekam\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``tchuekam dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\tchuekam\gateway-service\`` and sometimes
...
def _tchuekam_path_markers(tchuekam_home: Path) -> list[str]:
    """Path-entry substrings that identify TchuEkaM-owned User-PATH entries."""
    root = str(tchuekam_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare tchuekam-agent install dir.
    markers = [root + "\\tchuekam-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if TCHUEKAM_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "tchuekam".  We don't want to catch
    # unrelated entries like "ctchuekam-foo" or "ephermeral", so we look for
    # backslash-tchuekam as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(tchuekam_home: Path) -> list[str]:
    """Strip TchuEkaM-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _tchuekam_path_markers(tchuekam_home)
            kept: list[str] = []
...
def remove_tchuekam_env_vars_windows() -> list[str]:
    """Delete TCHUEKAM_HOME and TCHUEKAM_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("TCHUEKAM_HOME", "TCHUEKAM_GIT_BASH_PATH"):
                try:
...
def _is_default_tchuekam_home(tchuekam_home: Path) -> bool:
    """Return True when ``tchuekam_home`` points at the default (non-profile) root."""
    try:
        from tchuekam_constants import get_default_tchuekam_root
        return tchuekam_home.resolve() == get_default_tchuekam_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its TCHUEKAM_HOME directory.

    We shell out to ``tchuekam -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current TCHUEKAM_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m tchuekam_cli.main` so we don't depend on a `tchuekam`
    #    wrapper that may be half-removed mid-uninstall.
    tchuekam_invocation = [_sys.executable, "-m", "tchuekam_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                tchuekam_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's TCHUEKAM_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.tchuekam/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.tchuekam/ for future reinstall
    """
    project_root = get_project_root()
    tchuekam_home = get_tchuekam_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie TCHUEKAM_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_tchuekam_home(tchuekam_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ TchuEkaM Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {tchuekam_home / 'config.yaml'}")
    print(f"  Secrets: {tchuekam_home / '.env'}")
    print(f"  Data:    {tchuekam_home / 'cron/'}, {tchuekam_home / 'sessions/'}, {tchuekam_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their TCHUEKAM_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL TchuEkaM data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the TchuEkaM code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in tchuekam_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\tchuekam\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(tchuekam_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No TchuEkaM-owned PATH entries in User environment")

        log_info("Removing TCHUEKAM_HOME / TCHUEKAM_GIT_BASH_PATH User env vars...")
        removed_env = remove_tchuekam_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No TchuEkaM-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing tchuekam command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.tchuekam/, just remove the tchuekam-agent subdir
            if tchuekam_home in project_root.parents or project_root.parent == tchuekam_home:
    ...
    #     under TCHUEKAM_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(tchuekam_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(tchuekam_home)
    ...
    # 5. Optionally remove ~/.tchuekam/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile TCHUEKAM_HOME dirs live under
        ...
        try:
            if tchuekam_home.exists():
                shutil.rmtree(tchuekam_home)
                log_success(f"Removed {tchuekam_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {tchuekam_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {tchuekam_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {tchuekam_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/tchuekam-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/tchuekam-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using TchuEkaM Agent! ⚕")
    print()
