"""tchuekam hooks — inspect and manage shell-script hooks.

Usage::

    tchuekam hooks list
    tchuekam hooks test <event> [--for-tool X] [--payload-file F]
    tchuekam hooks revoke <command>
    tchuekam hooks doctor

Consent records live under ``~/.tchuekam/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.tchuekam/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``tchuekam hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: tchuekam hooks {list|test|revoke|doctor}")
        print("Run 'tchuekam hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.tchuekam/config.yaml.")
        print("See `tchuekam hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `tchuekam hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `tchuekam hooks test` and `tchuekam hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (TchuEkaM wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `tchuekam hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `tchuekam hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `tchuekam hooks doctor`.")
