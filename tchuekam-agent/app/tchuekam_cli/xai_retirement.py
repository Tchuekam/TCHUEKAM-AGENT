...
Pure logic: walks a TchuEkaM config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `tchuekam doctor` and a future `tchuekam migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a TchuEkaM config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a TchuEkaM config and return retirement issues.
