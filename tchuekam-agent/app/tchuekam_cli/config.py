"""
Configuration management for TchuEkaM Agent.

Config files are stored in ~/.tchuekam/ for easy access:
- ~/.tchuekam/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.tchuekam/.env         - API keys and secrets

This module provides:
- tchuekam config          - Show current configuration
- tchuekam config edit     - Open config in editor
- tchuekam config set      - Set a specific value
- tchuekam config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.tchuekam/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``tchuekam logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  tchuekam config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `tchuekam plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "TCHUEKAM_LANGFUSE_ENV",
    "TCHUEKAM_LANGFUSE_RELEASE",
    "TCHUEKAM_LANGFUSE_SAMPLE_RATE",
    "TCHUEKAM_LANGFUSE_MAX_CHARS",
    "TCHUEKAM_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from tchuekam_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("TCHUEKAM_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_tchuekam_home() / ".managed"
...
def is_managed() -> bool:
    """Check if TchuEkaM is running in package-manager-managed mode.

    Two signals: the TCHUEKAM_MANAGED env var (set by the systemd service),
    or a .managed marker file in TCHUEKAM_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade tchuekam-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how TchuEkaM was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.tchuekam/.install_method`` file (written by installers)
    2. TCHUEKAM_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_tchuekam_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.tchuekam/.install_method."""
    stamp = get_tchuekam_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade tchuekam-agent"
    if method == "docker":
        return "docker pull nousresearch/tchuekam-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade tchuekam-agent"
        return "pip install --upgrade tchuekam-agent"
    return "tchuekam update"
...
def format_managed_message(action: str = "modify this TchuEkaM installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("TCHUEKAM_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this TchuEkaM installation is managed by NixOS "
            f"(TCHUEKAM_MANAGED={env_hint}).\n"
            "Edit services.tchuekam-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this TchuEkaM installation is managed by Homebrew "
            f"(TCHUEKAM_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade tchuekam-agent"
        )

    return (
        f"Cannot {action}: this TchuEkaM installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall TchuEkaM."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from TCHUEKAM_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, tchuekam_bin
    or None if container mode is not active, we're already inside the
    container, or TCHUEKAM_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("TCHUEKAM_DEV") == "1":
        return None
    ...
    container_mode_file = get_tchuekam_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "tchuekam-agent")
    exec_user = info.get("exec_user", "tchuekam")
    tchuekam_bin = info.get("tchuekam_bin", "/data/current-package/bin/tchuekam")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "tchuekam_bin": tchuekam_bin,
    }
...
# Re-export from tchuekam_constants — canonical definition lives there.
from tchuekam_constants import get_tchuekam_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_tchuekam_home() / "config.yaml"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the tchuekam group can
    share state with the gateway service.

    The mode can be overridden via the TCHUEKAM_HOME_MODE environment variable
    (e.g. TCHUEKAM_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse TCHUEKAM_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("TCHUEKAM_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When TchuEkaM runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("TCHUEKAM_CONTAINER") or os.environ.get("TCHUEKAM_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set TCHUEKAM_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into TCHUEKAM_HOME if the user doesn't have one yet."""
...
def ensure_tchuekam_home():
    """Ensure ~/.tchuekam directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_tchuekam_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_tchuekam_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"TCHUEKAM_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the TchuEkaM-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, TchuEkaM auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), TchuEkaM sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when TchuEkaM
        ...
        # bundled TchuEkaM image, which drops to the `tchuekam` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, TchuEkaM sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and TchuEkaM should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``tchuekam chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.tchuekam/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: tchuekam sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, TchuEkaM truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``tchuekam kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``tchuekam kanban decompose`` and the kanban
        ...
        # ``tchuekam profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `tchuekam model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `tchuekam --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `tchuekam -c` muscle memory.  Default off so existing
        # users aren't surprised.  TCHUEKAM_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/tchuekam`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.tchuekam/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.tchuekam/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, TchuEkaM feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before TchuEkaM auto-pauses the goal and
    ...
    # always goes to ~/.tchuekam/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${TCHUEKAM_SKILL_DIR} and ${TCHUEKAM_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `tchuekam curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.tchuekam/skills/ into
        # ~/.tchuekam/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `tchuekam curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for tchuekam-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *TchuEkaM Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (TCHUEKAM_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.tchuekam/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or TCHUEKAM_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `tchuekam doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``tchuekam_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow TchuEkaM to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via TCHUEKAM_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `tchuekam -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `tchuekam` launches with
        ...
        # decomposition is manual via `tchuekam kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     tchuekam-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.tchuekam/logs/.
    ...
    # update model picker lists without shipping a tchuekam-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://tchuekam-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `tchuekam model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.tchuekam/state.db.
    ...
    "sessions": {
        ...
        # default of ``tchuekam sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.tchuekam/sessions/session_{sid}.json`` on every turn
    ...
    # ``tchuekam update`` behaviour.
    "updates": {
        # Run a full ``tchuekam backup``-style zip of TCHUEKAM_HOME before every
        # ``tchuekam update``.  Backups land in ``<TCHUEKAM_HOME>/backups/`` and
        # can be restored with ``tchuekam import <path>``.  Off by default —
        # on large TCHUEKAM_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<TCHUEKAM_HOME>/lsp/bin/`` on first use.
    ...
    # `tchuekam tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.tchuekam/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.tchuekam/.env (or your shell) and never in config.yaml.
            ...
            # ~/.tchuekam/bin/ on first use.  When False you must install
            ...
            # `tchuekam secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'tchuekam model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the TchuEkaM auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @tchuekam:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. TCHUEKAM_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #tchuekam)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: tchuekam-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'tchuekam-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote TchuEkaM API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote TchuEkaM API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote TchuEkaM API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # TCHUEKAM_TOOL_PROGRESS and TCHUEKAM_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "TCHUEKAM_TOOL_PROGRESS": {
    ...
    "TCHUEKAM_PREFILL_MESSAGES_FILE": {
    ...
    "TCHUEKAM_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.tchuekam.config`` entries, then checks
    ...
    try:
        ...
        # should never break `tchuekam update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older TchuEkaM versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``tchuekam_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'tchuekam setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — TchuEkaM won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'tchuekam doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("TCHUEKAM_HOME", "~/.tchuekam")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("TCHUEKAM_TOOL_PROGRESS")
            old_mode = get_env_value("TCHUEKAM_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from TCHUEKAM_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from TCHUEKAM_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("TCHUEKAM_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from TCHUEKAM_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$TCHUEKAM_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_tchuekam_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `tchuekam plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `tchuekam curator status` has no
    ...
    #   3. Creates `~/.tchuekam/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_tchuekam_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_tchuekam_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: tchuekam config set <key> <value>")
    ...
    # metadata.tchuekam.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _tchuekam_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.tchuekam/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.tchuekam/config.yaml.
    ...
    (which change ``TCHUEKAM_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — tchuekam auth) — OpenAI Codex
#   nous         (OAuth — tchuekam auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.tchuekam/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.tchuekam/.env.
    ...
    interactive menu render (`tchuekam tools`, `tchuekam setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `tchuekam tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real TchuEkaM env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.tchuekam/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.tchuekam/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.tchuekam/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.tchuekam/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to TchuEkaM — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known TchuEkaM vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.tchuekam/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ TchuEkaM Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  tchuekam config edit     # Edit config file", Colors.DIM))
    print(color("  tchuekam config set <key> <value>", Colors.DIM))
    print(color("  tchuekam setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: tchuekam config set <key> <value>")
            print()
            print("Examples:")
            print("  tchuekam config set model anthropic/claude-sonnet-4")
            print("  tchuekam config set terminal.backend docker")
            print("  tchuekam config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'tchuekam config migrate' to add them")
        ...
        print("  tchuekam config           Show current configuration")
        print("  tchuekam config edit      Open config in editor")
        print("  tchuekam config set <key> <value>   Set a config value")
        print("  tchuekam config check     Check for missing/outdated config")
        print("  tchuekam config migrate   Update config with new options")
        print("  tchuekam config path      Show config file path")
        print("  tchuekam config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``tchuekam config`` UI so users
