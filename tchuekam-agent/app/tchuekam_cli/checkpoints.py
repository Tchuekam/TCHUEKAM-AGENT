"""`tchuekam checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.tchuekam/checkpoints/``.  Actions:

    tchuekam checkpoints               # same as `status`
    tchuekam checkpoints status        # total size, project count, breakdown
    tchuekam checkpoints list          # per-project checkpoint counts + workdir
    tchuekam checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    tchuekam checkpoints clear [-f]    # nuke the entire base (asks first)
    tchuekam checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    tchuekam checkpoints
    tchuekam checkpoints prune --retention-days 3 --max-size-mb 200
    tchuekam checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: tchuekam checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``tchuekam checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `tchuekam checkpoints` → status
