"""tchuekam webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.tchuekam/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _tchuekam_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: tchuekam gateway run
"""
...
def webhook_command(args):
    """Entry point for 'tchuekam webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: tchuekam webhook {subscribe|list|remove|test}")
        print("Run 'tchuekam webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (tchuekam gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: tchuekam webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from tchuekam webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (tchuekam gateway run)")
