"""
Dump command for tchuekam CLI.

Outputs a compact, plain-text summary of the user's TchuEkaM setup
...
from tchuekam_cli.config import get_tchuekam_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``tchuekam dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from tchuekam_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(tchuekam_home: Path) -> int:
...
def _cron_summary(tchuekam_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = tchuekam_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"tchuekam_home:      {display_tchuekam_home()}")
    ...
    toolsets = config.get("toolsets", ["tchuekam-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(tchuekam_home)}")
    lines.append(f"  skills:             {_count_skills(tchuekam_home)}")
