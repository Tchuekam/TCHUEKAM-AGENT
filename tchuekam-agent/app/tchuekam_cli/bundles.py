"""Implementation of the ``tchuekam bundles`` CLI subcommand.

Mirrors the structure of ``tchuekam_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `tchuekam bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  tchuekam bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``tchuekam bundles`` argparse tree.

    Called from ``tchuekam_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `tchuekam bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``tchuekam bundles <subcommand>`` to the right handler."""
