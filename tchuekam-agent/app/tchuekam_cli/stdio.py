...
on Japanese locales, etc.).  TchuEkaM's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``tchuekam_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. TchuEkaM is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``TCHUEKAM_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("TCHUEKAM_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the TchuEkaM-managed Git install directories so
    ...
    # already-running shells don't see the change — which means tchuekam
    ...
    # install without TchuEkaM-managed PortableGit).
    ...
    # stdin is re-configured for completeness; TchuEkaM's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and TchuEkaM's
    ``tchuekam config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching TchuEkaM (User env var in Windows Settings, or
    export in a PowerShell profile) and TchuEkaM picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known TchuEkaM-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\tchuekam\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``tchuekam`` from right after install) retain their old PATH.

    Any subprocess TchuEkaM spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known TchuEkaM-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "tchuekam", "git", "cmd"),
        os.path.join(local_appdata, "tchuekam", "git", "bin"),
        os.path.join(local_appdata, "tchuekam", "git", "usr", "bin"),
        # TchuEkaM venv Scripts directory — host of the tchuekam.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes tchuekam, but harmless to include.
        os.path.join(local_appdata, "tchuekam", "tchuekam-agent", "venv", "Scripts"),
