...
Usage:
    tchuekam pairing list              # Show all pending + approved users
    tchuekam pairing approve <platform> <code>  # Approve a pairing code
    tchuekam pairing revoke <platform> <user_id> # Revoke user access
    tchuekam pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle tchuekam pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: tchuekam pairing {list|approve|revoke|clear-pending}")
        print("Run 'tchuekam pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.tchuekam/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'tchuekam pairing list' to see pending codes.\n")
