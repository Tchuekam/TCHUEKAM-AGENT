"""Slash command definitions and autocomplete for the TchuEkaM CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of TchuEkaM config/state", "Session",
    ...
    CommandDef("swe", "Delegate a task to the autonomous SWE agent (Closed-loop)", "Session",
               args_hint="<prompt>"),
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal TchuEkaM works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while TchuEkaM is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.tchuekam/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update TchuEkaM Agent to the latest version", "Info"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info"),

    # TCHUEKAM Core
    CommandDef("brief", "Compile and display the TCHUEKAM morning brief", "TCHUEKAM",
               cli_only=True),
    CommandDef("search", "Query the SQLite FTS5 file index instantaneously", "TCHUEKAM",
               args_hint="<query>", cli_only=True),
    CommandDef("index", "Trigger a background scan of local drives to refresh the file index", "TCHUEKAM",
               args_hint="[path...]", cli_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`tchuekam_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/tchuekam`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full TchuEkaM
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``tchuekam skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``tchuekam skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``tchuekam skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/tchuekam <verb>`` subcommand.
    ...
    can still reach them via ``/tchuekam <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/tchuekam`` is always reserved as the first entry so the
    legacy ``/tchuekam <subcommand>`` form keeps working for anything that
    ...
    # Reserve /tchuekam as the catch-all top-level command.
    entries.append(("tchuekam", "Talk to TchuEkaM or run a subcommand", "[subcommand] [args]"))
    seen.add("tchuekam")
...
def slack_app_manifest(request_url: str = "https://tchuekam-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /tchuekam handler.

    Maps both canonical names and aliases so /tchuekam bg do stuff works
    the same as /tchuekam background do stuff.

    Plugin-registered slash commands are included so ``/tchuekam <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from tchuekam_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
