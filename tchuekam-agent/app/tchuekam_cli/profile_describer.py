...
Used by ``tchuekam profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``tchuekam_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the TchuEkaM Agent kanban board.
...
Rules:
  ...
  - Never write "TchuEkaM Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_tchuekam_home())
        else:
