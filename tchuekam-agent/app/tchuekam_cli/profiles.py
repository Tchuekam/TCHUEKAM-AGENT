"""
Profile management for multiple isolated TchuEkaM instances.

Each profile is a fully independent TCHUEKAM_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.tchuekam/profiles/<name>/`` by default.

The "default" profile is ``~/.tchuekam`` itself — backward compatible,
zero migration needed.

Usage::

    tchuekam profile create coder          # fresh profile + bundled skills
    tchuekam profile create coder --clone  # also copy config, .env, SOUL.md, skills
    tchuekam profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    tchuekam -p coder chat                 # or via flag
    tchuekam profile use coder             # set as sticky default
    tchuekam profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See tchuekam_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.tchuekam``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   tchuekam-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `tchuekam profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `tchuekam
...
# `tchuekam skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         TchuEkaM infrastructure directories that only the default profile
         (``~/.tchuekam``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_tchuekam_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.tchuekam) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "tchuekam-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "tchuekam", "default", "test", "tmp", "root", "sudo",
})

# TchuEkaM subcommands that cannot be used as profile names/aliases
_TCHUEKAM_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the tchuekam root, NOT to the current TCHUEKAM_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where TCHUEKAM_HOME points outside
    ``~/.tchuekam``, profiles live under ``TCHUEKAM_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_tchuekam_home() / "profiles"


def _get_default_tchuekam_home() -> Path:
    """Return the default (pre-profile) TCHUEKAM_HOME path.

    In standard deployments this is ``~/.tchuekam``.
    In Docker/custom deployments where TCHUEKAM_HOME is outside ``~/.tchuekam``
    (e.g. ``/opt/data``), returns TCHUEKAM_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_tchuekam_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the TchuEkaM installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its TCHUEKAM_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_tchuekam_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, tchuekam subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _TCHUEKAM_SUBCOMMANDS:
        return f"'{canon}' conflicts with a tchuekam subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``tchuekam profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing TchuEkaM config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_tchuekam_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``tchuekam update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.tchuekam)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.tchuekam/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `tchuekam update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`tchuekam profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `tchuekam update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `tchuekam profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches TCHUEKAM_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``tchuekam profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "TCHUEKAM_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.tchuekam).\n"
            "To remove everything, use: tchuekam uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set TCHUEKAM_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("TCHUEKAM_HOME")
    try:
        os.environ["TCHUEKAM_HOME"] = str(profile_dir)
        from tchuekam_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["TCHUEKAM_HOME"] = old_home
        elif "TCHUEKAM_HOME" in os.environ:
            del os.environ["TCHUEKAM_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.tchuekam/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: tchuekam profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from TCHUEKAM_HOME.

    Returns ``"default"`` if TCHUEKAM_HOME is not set or points to ``~/.tchuekam``.
    Returns the profile name if TCHUEKAM_HOME points into ``~/.tchuekam/profiles/<name>``.
    Returns ``"custom"`` if TCHUEKAM_HOME is set to an unrecognized path.
    """
    from tchuekam_constants import get_tchuekam_home
    tchuekam_home = get_tchuekam_home()
    resolved = tchuekam_home.resolve()

    default_resolved = _get_default_tchuekam_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.tchuekam itself — its parent is ~/ and its
        # directory name is ".tchuekam", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: tchuekam profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.tchuekam itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.tchuekam). "
            "Specify a different name: tchuekam profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="tchuekam_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"tchuekam.{old_name}"
    new_host = f"tchuekam.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_tchuekam_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a TCHUEKAM_HOME path string.

    Called early in the CLI entry point, before any tchuekam modules
    are imported, to set the TCHUEKAM_HOME environment variable.
    """
