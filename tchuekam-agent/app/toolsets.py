...
_TCHUEKAM_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # File manipulation
    "read_file", "write_file", "patch", "search_files", "tchuekam_index_search",
    ...
    "text_to_speech",
    # Planning & memory
    "todo", "memory", "tchuekam_memory", "tchuekam_brief",
    ...
    # spawned as a kanban worker (TCHUEKAM_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `tchuekam tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``tchuekam tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (TCHUEKAM_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full TchuEkaM toolsets (CLI + messaging platforms)
    #
    ...
    "tchuekam-cli": {
        ...
        "tools": _TCHUEKAM_CORE_TOOLS,
        "includes": []
    },

    "tchuekam-cron": {
        # Mirrors tchuekam-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `tchuekam tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as tchuekam-cli; gated by `tchuekam tools`",
        "tools": _TCHUEKAM_CORE_TOOLS,
        "includes": []
    },
    ...
    "tchuekam-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _TCHUEKAM_CORE_TOOLS,
        "includes": []
    },

    "tchuekam-homeassistant": {
    ...
    "tchuekam-email": {
        "description": "Email bot toolset - interact with TchuEkaM via email (IMAP/SMTP)",
        "tools": _TCHUEKAM_CORE_TOOLS,
        "includes": []
    },
    ...
    "tchuekam-sms": {
        "description": "SMS bot toolset - interact with TchuEkaM via SMS (Twilio)",
        "tools": _TCHUEKAM_CORE_TOOLS,
        "includes": []
    },
    ...
    "tchuekam-webhook": {
        ...
        "tools": _TCHUEKAM_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "tchuekam-gateway": {
        ...
        "tools": [],
        "includes": ["tchuekam-telegram", "tchuekam-discord", "tchuekam-whatsapp", "tchuekam-slack", "tchuekam-signal", "tchuekam-bluebubbles", "tchuekam-homeassistant", "tchuekam-email", "tchuekam-sms", "tchuekam-mattermost", "tchuekam-matrix", "tchuekam-dingtalk", "tchuekam-feishu", "tchuekam-wecom", "tchuekam-wecom-callback", "tchuekam-weixin", "tchuekam-qqbot", "tchuekam-webhook", "tchuekam-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (tchuekam-<name>).
        # Gives them _TCHUEKAM_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("tchuekam-"):
            platform_name = name[len("tchuekam-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_TCHUEKAM_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
