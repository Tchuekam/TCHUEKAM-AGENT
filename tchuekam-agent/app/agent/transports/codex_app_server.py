...
projection into TchuEkaM' display, approval bridging, transcript projection into
...
"codex_app_server"`. TchuEkaM' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("TCHUEKAM_KANBAN_TASK"):
            kanban_db = spawn_env.get("TCHUEKAM_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "TCHUEKAM_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("TCHUEKAM_HOME", os.path.expanduser("~/.tchuekam")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "tchuekam",
        client_title: str = "TchuEkaM Agent",
