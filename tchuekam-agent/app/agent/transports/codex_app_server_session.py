...
Owns one Codex thread per TchuEkaM session. Drives `turn/start`, consumes
...
# TchuEkaM' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_TCHUEKAM_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse TchuEkaM/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per TchuEkaM session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _TCHUEKAM_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("TCHUEKAM_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="tchuekam",
            client_title="TchuEkaM Agent",
            client_version=_get_tchuekam_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but TchuEkaM' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into TchuEkaM' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own tchuekam-tools callback we
            # auto-accept — the user already approved TchuEkaM' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map TchuEkaM approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    TchuEkaM returns 'once', 'session', 'always', or 'deny'.
...
def _get_tchuekam_version() -> str:
    """Best-effort TchuEkaM version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("tchuekam-agent")
