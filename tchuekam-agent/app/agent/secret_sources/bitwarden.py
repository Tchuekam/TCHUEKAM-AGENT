...
TchuEkaM pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.tchuekam/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<tchuekam_home>/bin/bws`` on
  first use.  TchuEkaM pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.tchuekam/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``tchuekam`` invocations don't
  hammer the API.
* Failures NEVER block TchuEkaM startup.  Missing binary, no network,
...
# In-process cache so repeated load_tchuekam_dotenv() calls (CLI startup,
...
def _tchuekam_bin_dir() -> Path:
    """Where TchuEkaM stores its managed binaries.  Profile-aware."""
    from tchuekam_constants import get_tchuekam_home

    return get_tchuekam_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<tchuekam_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _tchuekam_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``tchuekam secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _tchuekam_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="tchuekam-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "tchuekam-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from tchuekam_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_tchuekam_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `tchuekam secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `tchuekam secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `tchuekam secrets bitwarden setup` to install."
        )
        return result
