...
Plugins built on TchuEkaM Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing TchuEkaM
...
:class:`~tchuekam_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors TchuEkaM' main config
...
handles every provider, fallback chain, and per-task override TchuEkaM
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`tchuekam_cli.plugins.PluginContext`
