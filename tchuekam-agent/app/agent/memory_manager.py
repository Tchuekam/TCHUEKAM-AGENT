...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``tchuekam_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_tchuekam_home()`` themselves.
        """
        if "tchuekam_home" not in kwargs:
            from tchuekam_constants import get_tchuekam_home
            kwargs["tchuekam_home"] = str(get_tchuekam_home())
        for provider in self._providers:
            try:
