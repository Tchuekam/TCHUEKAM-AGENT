...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time TchuEkaM starts and finds ``~/.openclaw/``.

    Points users at ``tchuekam claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``tchuekam claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to TchuEkaM, run "
        "`tchuekam claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `tchuekam claw cleanup` (renames it to ~/.openclaw.pre-migration — "
