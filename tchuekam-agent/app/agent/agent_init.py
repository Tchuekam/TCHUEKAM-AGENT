...
from tchuekam_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.tchuekam/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from tchuekam_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.tchuekam/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from tchuekam_logging import setup_logging, setup_verbose_logging
    setup_logging(tchuekam_home=_ra()._tchuekam_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that tchuekam_logging.setup_logging() does not
        ...
        # handler level inside tchuekam_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from tchuekam_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from tchuekam_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `tchuekam model` to "
                        "select a provider, or run `tchuekam setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff TCHUEKAM_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["TCHUEKAM_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.tchuekam/sessions/ alongside gateway sessions
    tchuekam_home = get_tchuekam_home()
    agent.logs_dir = tchuekam_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.tchuekam/sessions/session_{sid}.json)
    ...
    try:
        from tchuekam_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from tchuekam_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "tchuekam"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from tchuekam_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from tchuekam_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by TchuEkaM Agent.  Choose a model with at least "
