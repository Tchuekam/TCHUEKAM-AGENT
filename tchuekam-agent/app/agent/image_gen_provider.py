...
as ``kind: backend``) or ``~/.tchuekam/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$TCHUEKAM_HOME/cache/images/``, creating parents as needed."""
    from tchuekam_constants import get_tchuekam_home

    path = get_tchuekam_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$TCHUEKAM_HOME/cache/images/``.
