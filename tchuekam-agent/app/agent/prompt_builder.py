...
from tchuekam_constants import get_tchuekam_home, get_skills_dir, is_wsl
...
_TCHUEKAM_MD_NAMES = (".tchuekam.md", "TCHUEKAM.md")


def _find_tchuekam_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.tchuekam.md`` or ``TCHUEKAM.md``.
    ...
    for directory in [current, *current.parents]:
        for name in _TCHUEKAM_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are strategic, precise, and enterprise-grade in every interaction. "
    "You carry context and memory across conversations by executing `session_search` at startup to scan past transcripts, learning and training on past interactions. "
    "You permanently record project conventions in `tchuekam_memory` and locate local files and folders instantly via `tchuekam_index_search`. "
    "You communicate with authority and clarity. You are direct, not verbose. "
    "You admit uncertainty when appropriate. You are an autonomous intelligence platform "
    "— not an assistant. Think mission-critical. Operate accordingly."
)

TCHUEKAM_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using TCHUEKAM-AGENT "
    "itself, load the `tchuekam-agent` skill with skill_view(name='tchuekam-agent') "
    "before answering."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.tchuekam/kanban.db`. Your task id is in "
    "`$TCHUEKAM_KANBAN_TASK`; your workspace is `$TCHUEKAM_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $TCHUEKAM_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `tchuekam kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the TCHUEKAM-AGENT WebUI, a browser-based command interface. "
...
# container / remote host rather than on the machine where TchuEkaM itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host TchuEkaM runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where TchuEkaM itself is running. The host OS, home, and cwd "
                f"of the TchuEkaM process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where TchuEkaM "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_tchuekam_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.tchuekam/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("TCHUEKAM_PLATFORM")
        or get_session_env("TCHUEKAM_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot TCHUEKAM-AGENT itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `tchuekam-agent` skill "
            "first. It has the actual commands (e.g. `tchuekam config set …`, `tchuekam tools`, "
            "`tchuekam setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: tchuekam setup, tchuekam setup tools, tchuekam setup terminal, tchuekam status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from TCHUEKAM_HOME and return its content, or None.
    ...
    """
    try:
        from tchuekam_cli.config import ensure_tchuekam_home
        ensure_tchuekam_home()
    except Exception as e:
        logger.debug("Could not ensure TCHUEKAM_HOME before loading SOUL.md: %s", e)

    soul_path = get_tchuekam_home() / "SOUL.md"
...
def _load_tchuekam_md(cwd_path: Path) -> str:
    """.tchuekam.md / TCHUEKAM.md — walk to git root."""
    tchuekam_md_path = _find_tchuekam_md(cwd_path)
    if not tchuekam_md_path:
        return ""
    try:
        content = tchuekam_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = tchuekam_md_path.name
        try:
            rel = str(tchuekam_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".tchuekam.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", tchuekam_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .tchuekam.md / TCHUEKAM.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from TCHUEKAM_HOME is independent and always included when present.
    ...
    # SOUL.md from TCHUEKAM_HOME only — skip when already loaded as identity
    if not skip_soul:
