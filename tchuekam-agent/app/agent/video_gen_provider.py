...
as ``kind: backend``) or ``~/.tchuekam/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``tchuekam tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``tchuekam tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``tchuekam tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``tchuekam tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$TCHUEKAM_HOME/cache/videos/``, creating parents as needed."""
    from tchuekam_constants import get_tchuekam_home

    path = get_tchuekam_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$TCHUEKAM_HOME/cache/videos/``.
