...
Storage
-------
Bundles live in ``~/.tchuekam/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``TCHUEKAM_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``tchuekam bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under TCHUEKAM_HOME.

    Honors ``TCHUEKAM_BUNDLES_DIR`` for tests; falls back to
    ``<TCHUEKAM_HOME>/skill-bundles``.
    """
    override = os.environ.get("TCHUEKAM_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_tchuekam_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `tchuekam bundles` CLI subcommand.
# ---------------------------------------------------------------------------
