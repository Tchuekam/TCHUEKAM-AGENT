"""Anthropic Messages API adapter for TchuEkaM Agent.

Translates between TchuEkaM's internal OpenAI-style message format and
...
# TchuEkaM effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See tchuekam-agent#13848, #17057.
    """
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    TchuEkaM historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by TchuEkaM)
    ...
    # 1. TchuEkaM-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration TchuEkaM configs.
...
# ── TchuEkaM-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.tchuekam/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_TCHUEKAM_OAUTH_FILE = get_tchuekam_home() / ".anthropic_oauth.json"
...
def run_tchuekam_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run TchuEkaM-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize TchuEkaM with your Claude Pro/Max subscription.")
    print()
...
def read_tchuekam_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read TchuEkaM-managed OAuth credentials from ~/.tchuekam/.anthropic_oauth.json."""
    if _TCHUEKAM_OAUTH_FILE.exists():
        try:
            data = json.loads(_TCHUEKAM_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read TchuEkaM OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See tchuekam-agent#13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See tchuekam-agent#13848 (Kimi) and
    tchuekam-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("TchuEkaM Agent", "Claude Code")
                text = text.replace("TchuEkaM agent", "Claude Code")
                text = text.replace("tchuekam-agent", "claude-code")
    ...
    # silently hides reasoning text that TchuEkaM surfaces in its CLI. We
