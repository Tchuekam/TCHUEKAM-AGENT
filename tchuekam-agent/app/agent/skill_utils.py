...
from tchuekam_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``TCHUEKAM_PLATFORM`` or
            ``TCHUEKAM_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("TCHUEKAM_PLATFORM")
...
# which becomes the dominant cost of ``tchuekam`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.tchuekam/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``tchuekam`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against TCHUEKAM_HOME, not cwd
        if not p.is_absolute():
            p = (tchuekam_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.tchuekam/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    tchuekam = metadata.get("tchuekam") or {}
    if not isinstance(tchuekam, dict):
        tchuekam = {}
    return {
        "fallback_for_toolsets": tchuekam.get("fallback_for_toolsets", []),
        "requires_toolsets": tchuekam.get("requires_toolsets", []),
        "fallback_for_tools": tchuekam.get("fallback_for_tools", []),
        "requires_tools": tchuekam.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    tchuekam = metadata.get("tchuekam")
    if not isinstance(tchuekam, dict):
        return []
    raw = tchuekam.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes TchuEkaM metadata, VCS, virtualenv/dependency, and cache
