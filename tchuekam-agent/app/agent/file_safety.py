...
def _tchuekam_home_path() -> Path:
    """Resolve the active TCHUEKAM_HOME (profile-aware) without circular imports."""
    try:
        from tchuekam_constants import get_tchuekam_home  # local import to avoid cycles
        return get_tchuekam_home()
    except Exception:
        return Path(os.path.expanduser("~/.tchuekam"))


def _tchuekam_root_path() -> Path:
    """Resolve the TchuEkaM root dir (always the parent of any profile, never per-profile)."""
    try:
        from tchuekam_constants import get_default_tchuekam_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    tchuekam_home = _tchuekam_home_path()
    tchuekam_root = _tchuekam_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(tchuekam_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(tchuekam_root / ".env"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved TCHUEKAM_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("TCHUEKAM_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # TchuEkaM control-plane files: block both the ACTIVE profile's view
    # (tchuekam_home) AND the global root view. Without the root pass, a
    ...
    tchuekam_dirs = []
    for base in (_tchuekam_home_path(), _tchuekam_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in tchuekam_dirs:
                tchuekam_dirs.append(real)
        except Exception:
            continue

    for base_real in tchuekam_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied TchuEkaM path.

    Two categories are blocked:

      * Internal TchuEkaM cache files under ``TCHUEKAM_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under TCHUEKAM_HOME and the global TchuEkaM
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.tchuekam/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active TCHUEKAM_HOME (profile-aware) AND the global
    # TchuEkaM root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (TCHUEKAM_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    tchuekam_dirs: list[Path] = []
    for base in (_tchuekam_home_path(), _tchuekam_root_path()):
        try:
            real = base.resolve()
            if real not in tchuekam_dirs:
    ...
    for hd in tchuekam_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal TchuEkaM cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # TCHUEKAM_HOME or <root>.
    ...
    for hd in tchuekam_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a TchuEkaM credential store "
    ...
    for hd in tchuekam_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the TchuEkaM MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a TchuEkaM MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# TchuEkaM profiles are separate TCHUEKAM_HOME dirs under
...
#
# Reference: May 2026 incident where a tchuekam-security profile session
# edited skills under both ``~/.tchuekam/profiles/tchuekam-security/skills/``
# AND ``~/.tchuekam/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under TCHUEKAM_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from TCHUEKAM_HOME.

    ``~/.tchuekam``              -> ``"default"``
    ``~/.tchuekam/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _tchuekam_home_path().resolve()
        root_real = _tchuekam_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside TchuEkaM scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    TchuEkaM entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to TchuEkaM profile {info['target_profile']!r}, but the "
