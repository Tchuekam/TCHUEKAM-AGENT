"""Windows UTF-8 bootstrap for TchuEkaM entry points.
...
should be imported at the very top of every TchuEkaM entry point
(``tchuekam``, ``tchuekam-agent``, ``tchuekam-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import tchuekam_bootstrap``
# (or ``from tchuekam_bootstrap import apply_windows_utf8_bootstrap``) at
