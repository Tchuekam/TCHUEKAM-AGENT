...
# 5. Symlinks the 'tchuekam' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ TchuEkaM Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/tchuekam-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink tchuekam into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up tchuekam command..."

TCHUEKAM_BIN="$SCRIPT_DIR/venv/bin/tchuekam"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$TCHUEKAM_BIN" "$COMMAND_LINK_DIR/tchuekam"
echo -e "${GREEN}✓${NC} Symlinked tchuekam → $COMMAND_LINK_DISPLAY_DIR/tchuekam"
...
# ============================================================================
# Seed bundled skills into ~/.tchuekam/skills/
# ============================================================================

TCHUEKAM_SKILLS_DIR="${TCHUEKAM_HOME:-$HOME/.tchuekam}/skills"
mkdir -p "$TCHUEKAM_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.tchuekam/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$TCHUEKAM_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  tchuekam status        # Check configuration"
if is_termux; then
    echo "  tchuekam gateway       # Run gateway in foreground"
else
    echo "  tchuekam gateway install # Install gateway service (messaging + cron)"
fi
echo "  tchuekam cron list     # View scheduled jobs"
echo "  tchuekam doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m tchuekam_cli.main setup
fi
