...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    tchuekam_state = ModuleType("tchuekam_state")
    tchuekam_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "tchuekam_state", tchuekam_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "TchuEkaM"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "TchuEkaM"
