...
class TestTchuEkaMConstantsFallback:
    """Tests for _tchuekam_home.py fallback when tchuekam_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_tchuekam_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _tchuekam_home.py with tchuekam_constants blocked."""
        monkeypatch.setitem(sys.modules, "tchuekam_constants", None)
        spec = importlib.util.spec_from_file_location("_tchuekam_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_tchuekam_home_env_var(self, monkeypatch, tmp_path):
        """When tchuekam_constants is missing, TCHUEKAM_HOME comes from env var."""
        monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path / "custom-tchuekam"))
        module = self._load_helper(monkeypatch)
        assert module.get_tchuekam_home() == tmp_path / "custom-tchuekam"

    def test_fallback_defaults_to_dot_tchuekam(self, monkeypatch):
        """When tchuekam_constants is missing and TCHUEKAM_HOME unset, default to ~/.tchuekam."""
        monkeypatch.delenv("TCHUEKAM_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_tchuekam_home() == Path.home() / ".tchuekam"

    def test_fallback_ignores_empty_tchuekam_home(self, monkeypatch):
        """Empty/whitespace TCHUEKAM_HOME is treated as unset."""
        monkeypatch.setenv("TCHUEKAM_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_tchuekam_home() == Path.home() / ".tchuekam"

    def test_fallback_display_tchuekam_home_shortens_path(self, monkeypatch):
        """Fallback display_tchuekam_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("TCHUEKAM_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_tchuekam_home() == "~/.tchuekam"

    def test_fallback_display_tchuekam_home_profile_path(self, monkeypatch):
        """Fallback display_tchuekam_home() handles profile paths under ~/."""
        monkeypatch.setenv("TCHUEKAM_HOME", str(Path.home() / ".tchuekam/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_tchuekam_home() == "~/.tchuekam/profiles/coder"

    def test_fallback_display_tchuekam_home_custom_path(self, monkeypatch):
        """Fallback display_tchuekam_home() returns full path for non-home locations."""
        monkeypatch.setenv("TCHUEKAM_HOME", "/opt/tchuekam-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_tchuekam_home() == "/opt/tchuekam-custom"

    def test_delegates_to_tchuekam_constants_when_available(self):
        """When tchuekam_constants IS importable, _tchuekam_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_tchuekam_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import tchuekam_constants
        assert module.get_tchuekam_home is tchuekam_constants.get_tchuekam_home
        assert module.display_tchuekam_home is tchuekam_constants.display_tchuekam_home
