...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. TchuEkaM passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("TCHUEKAM_HOME", "/users/alice/.tchuekam/profiles/backend-worker")
        monkeypatch.setenv("TCHUEKAM_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "TCHUEKAM_KANBAN_DB",
            "/users/alice/.tchuekam/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.tchuekam/kanban/boards/smoke"]'
            in cmd
        )
