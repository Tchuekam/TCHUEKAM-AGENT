...
def test_tchuekam_client_tag_includes_current_version():
    """The client tag must reflect tchuekam_cli.__version__ verbatim."""
    from tchuekam_cli import __version__
    from agent.portal_tags import tchuekam_client_tag

    assert tchuekam_client_tag() == f"client=tchuekam-client-v{__version__}"


def test_tchuekam_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import tchuekam_client_tag

    tag = tchuekam_client_tag()
    assert tag.startswith("client=tchuekam-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import tchuekam_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=tchuekam-agent" in tags
    assert tchuekam_client_tag() in tags
    assert len(tags) == 2
