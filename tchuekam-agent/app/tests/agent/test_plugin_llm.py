...
""",
        ...
        from tchuekam_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        (tchuekam_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))
        from tchuekam_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from tchuekam_cli.plugins import PluginContext, PluginManifest, PluginManager
