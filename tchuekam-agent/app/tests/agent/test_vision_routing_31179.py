...
def isolated_home(monkeypatch):
    """Temp TCHUEKAM_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="tchuekam_test_31179_")
    tchuekam_home = os.path.join(test_home, ".tchuekam")
    os.makedirs(tchuekam_home)
...
def _fresh_modules():
    """Drop cached tchuekam modules so each test reloads against current env."""
