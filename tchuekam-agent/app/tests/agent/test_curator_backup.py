...
def backup_env(monkeypatch, tmp_path):
    """Isolate TCHUEKAM_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_tchuekam_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd tchuekam_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under TCHUEKAM_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level TCHUEKAM_DIR picks up the tmp HOME."""
    import tchuekam_constants
    importlib.reload(tchuekam_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.tchuekam/cron/jobs.json
