...
def _clean_state():
    ...
    saved = {}
    for k in ("TCHUEKAM_INTERACTIVE", "TCHUEKAM_GATEWAY_SESSION", "TCHUEKAM_EXEC_ASK", "TCHUEKAM_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["TCHUEKAM_INTERACTIVE"] = "1"
        session_key = os.getenv("TCHUEKAM_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No TCHUEKAM_INTERACTIVE or TCHUEKAM_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
