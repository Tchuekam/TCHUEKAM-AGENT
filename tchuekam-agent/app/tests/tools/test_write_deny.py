...
class TestWriteDenyExactPaths:
    ...
    def test_tchuekam_env(self):
        # ``.env`` under the active TCHUEKAM_HOME (profile-aware, not just
        # ``~/.tchuekam``) must be write-denied. The hermetic test conftest
        # points TCHUEKAM_HOME at a tempdir — resolve via get_tchuekam_home()
        # to match the denylist.
        from tchuekam_constants import get_tchuekam_home
        path = str(get_tchuekam_home() / ".env")
        assert _is_write_denied(path) is True

    def test_tchuekam_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.tchuekam/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "tchuekam_root"
        ...
        # Sanity check: TCHUEKAM_HOME does point to the profile dir, not the root.
        from tchuekam_constants import get_tchuekam_home, get_default_tchuekam_root
        assert get_tchuekam_home() == profile_home
        assert get_default_tchuekam_root() == root
...
class TestWriteAllowed:
    ...
    def test_tchuekam_config_not_env(self):
        path = os.path.join(str(Path.home()), ".tchuekam", "config.yaml")
