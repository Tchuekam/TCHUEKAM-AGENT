...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because tchuekam uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("TCHUEKAM_GATEWAY_SESSION", "1")
        monkeypatch.delenv("TCHUEKAM_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("TCHUEKAM_INTERACTIVE", raising=False)
        monkeypatch.delenv("TCHUEKAM_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("TCHUEKAM_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("TCHUEKAM_INTERACTIVE", false_like_value)
        monkeypatch.delenv("TCHUEKAM_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("TCHUEKAM_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["TCHUEKAM_INTERACTIVE", "TCHUEKAM_GATEWAY_SESSION", "TCHUEKAM_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("TCHUEKAM_INTERACTIVE", "TCHUEKAM_GATEWAY_SESSION", "TCHUEKAM_EXEC_ASK"):
