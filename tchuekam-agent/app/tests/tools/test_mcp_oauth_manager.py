...
def test_tchuekam_provider_subclass_exists():
    """TchuEkaMMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _TCHUEKAM_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _TCHUEKAM_PROVIDER_CLS is not None
    assert issubclass(_TCHUEKAM_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_tchuekam_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns TchuEkaMMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _TCHUEKAM_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path))
    ...
    assert _TCHUEKAM_PROVIDER_CLS is not None
    assert isinstance(provider, _TCHUEKAM_PROVIDER_CLS)
    assert provider._tchuekam_server_name == "srv"
