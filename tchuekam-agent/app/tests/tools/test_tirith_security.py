...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._tchuekam_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_tchuekam_bin(self):
        """After _INSTALL_FAILED, manual install in TCHUEKAM_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        tchuekam_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(tchuekam_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(tchuekam_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._tchuekam_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == tchuekam_bin
            assert _tirith_mod._resolved_path == tchuekam_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# TCHUEKAM_HOME isolation
# ---------------------------------------------------------------------------

class TestTchuEkaMHomeIsolation:
    def test_tchuekam_bin_dir_respects_tchuekam_home(self):
        """_tchuekam_bin_dir must use TCHUEKAM_HOME, not hardcoded ~/.tchuekam."""
        from tools.tirith_security import _tchuekam_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"TCHUEKAM_HOME": tmpdir}):
            result = _tchuekam_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_tchuekam_home(self):
        """_failure_marker_path must use TCHUEKAM_HOME, not hardcoded ~/.tchuekam."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"TCHUEKAM_HOME": "/custom/tchuekam"}):
            result = _failure_marker_path()
        assert result == "/custom/tchuekam/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets TCHUEKAM_HOME; verify it's active."""
        tchuekam_home = os.getenv("TCHUEKAM_HOME")
        assert tchuekam_home is not None, "TCHUEKAM_HOME should be set by conftest"
        assert "tchuekam_test" in tchuekam_home, "Should point to test temp dir"

    def test_get_tchuekam_home_fallback(self):
        """Without TCHUEKAM_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_tchuekam_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove TCHUEKAM_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".tchuekam")
            result = _get_tchuekam_home()
