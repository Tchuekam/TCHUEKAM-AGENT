...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_tchuekam_home_override(self, tmp_path):
        """Auth lookup should read from TCHUEKAM_HOME/auth.json, not ~/.tchuekam/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".tchuekam").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "TCHUEKAM_HOME": str(tchuekam_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``tchuekam tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
