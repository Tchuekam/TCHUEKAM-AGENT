...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        tchuekam_home.mkdir()
        (tchuekam_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(tchuekam_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.tchuekam/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        (tchuekam_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        (tchuekam_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        (tchuekam_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        tchuekam_home = tmp_path / ".tchuekam"
        skills_dir = tchuekam_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.tchuekam/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        tchuekam_home = tmp_path / ".tchuekam"
        (tchuekam_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"TCHUEKAM_HOME": str(tchuekam_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.tchuekam")

        assert mounts[0]["container_path"] == "/home/user/.tchuekam/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.tchuekam/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.tchuekam/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape TCHUEKAM_HOME."""
        monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path / ".tchuekam"))
        (tmp_path / ".tchuekam").mkdir()

        # Create a sensitive file one level above tchuekam_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside tchuekam_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside TCHUEKAM_HOME must still be registered."""
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))
        (tchuekam_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_tchuekam_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of TCHUEKAM_HOME must be allowed."""
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        subdir = tchuekam_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside TCHUEKAM_HOME pointing outside must be rejected."""
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))

        # Create a sensitive file outside tchuekam_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside tchuekam_home pointing outside
        symlink = tchuekam_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes TCHUEKAM_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, tchuekam_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape TCHUEKAM_HOME."""
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))
        ...
        sensitive.write_text("{}")
        self._write_config(tchuekam_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(tchuekam_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside TCHUEKAM_HOME via config must still mount."""
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))

        (tchuekam_home / "oauth.json").write_text("{}")
        self._write_config(tchuekam_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        (tchuekam_home / "cache" / "documents").mkdir(parents=True)
        (tchuekam_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.tchuekam/cache/documents" in paths
        assert "/root/.tchuekam/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.tchuekam/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        # Use legacy dir name — get_tchuekam_dir prefers old if it exists
        (tchuekam_home / "document_cache").mkdir()
        (tchuekam_home / "image_cache").mkdir()
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(tchuekam_home / "document_cache") in host_paths
        assert str(tchuekam_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.tchuekam/cache/documents" in container_paths
        assert "/root/.tchuekam/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        tchuekam_home = tmp_path / ".tchuekam"
        doc_dir = tchuekam_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        tchuekam_home = tmp_path / ".tchuekam"
        ss_dir = tchuekam_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.tchuekam/cache/screenshots/session_abc/screen1.png"
