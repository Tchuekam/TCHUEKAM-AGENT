...
def _restore_tool_modules():
    original_tchuekam_home = os.environ.get("TCHUEKAM_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "tchuekam_cli"
        or name.startswith("tchuekam_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_tchuekam_home is None:
            os.environ.pop("TCHUEKAM_HOME", None)
        else:
            os.environ["TCHUEKAM_HOME"] = original_tchuekam_home
        _reset_modules(("tools", "tchuekam_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    tchuekam_cli = types.ModuleType("tchuekam_cli")
    tchuekam_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["tchuekam_cli"] = tchuekam_cli
    tchuekam_home = tmp_path / "tchuekam-home"
    os.environ["TCHUEKAM_HOME"] = str(tchuekam_home)
    sys.modules["tchuekam_cli.config"] = types.SimpleNamespace(
        get_tchuekam_home=lambda: tchuekam_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="tchuekam-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": tchuekam_home / "modal_snapshots.json",
