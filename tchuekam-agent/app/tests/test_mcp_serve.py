"""
Tests for mcp_serve — TchuEkaM MCP server.
...
def _isolate_tchuekam_home(tmp_path, monkeypatch):
    """Redirect TCHUEKAM_HOME to a temp directory."""
    monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path))
    try:
        import tchuekam_constants
        monkeypatch.setattr(tchuekam_constants, "get_tchuekam_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking tchuekam_state schema."""
