...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.tchuekam/.update_response."""
        adapter = self._make_adapter()
        tchuekam_home = tmp_path / "tchuekam_home"
        tchuekam_home.mkdir()
        monkeypatch.setattr(
            "tchuekam_constants.get_tchuekam_home",
            lambda: tchuekam_home,
        )
        ...
        response = tchuekam_home / ".update_response"
