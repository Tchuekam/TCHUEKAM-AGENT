...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.tchuekam."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /tchuekam, /btw, /stop,
        ...
        for expected in ("/tchuekam", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@tchuekam`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@tchuekam"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@tchuekam")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /tchuekam subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /tchuekam btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_tchuekam_prefix_still_works(self, adapter):
        """Backward compat: /tchuekam btw foo must still route to /btw foo.

        Old workspace manifests only declared /tchuekam as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_tchuekam_freeform_question(self, adapter):
        """/tchuekam <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/tchuekam",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_tchuekam_slash_stashes_context(self, adapter):
        """Legacy /tchuekam <subcommand> also stashes context."""
        command = {
            "command": "/tchuekam",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_tchuekam_question_does_not_stash_context(self, adapter):
        """Free-form /tchuekam <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/tchuekam",
