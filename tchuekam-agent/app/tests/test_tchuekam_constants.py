"""Tests for tchuekam_constants module."""
...
class TestGetDefaultTchuEkaMRoot:
    """Tests for get_default_tchuekam_root() — Docker/custom deployment awareness."""

    def test_no_tchuekam_home_returns_native(self, tmp_path, monkeypatch):
        """When TCHUEKAM_HOME is not set, returns ~/.tchuekam."""
        monkeypatch.delenv("TCHUEKAM_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_tchuekam_root() == tmp_path / ".tchuekam"

    def test_tchuekam_home_is_native(self, tmp_path, monkeypatch):
        """When TCHUEKAM_HOME = ~/.tchuekam, returns ~/.tchuekam."""
        native = tmp_path / ".tchuekam"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(native))
        assert get_default_tchuekam_root() == native

    def test_tchuekam_home_is_profile(self, tmp_path, monkeypatch):
        """When TCHUEKAM_HOME is a profile under ~/.tchuekam, returns ~/.tchuekam."""
        native = tmp_path / ".tchuekam"
    ...
    def test_tchuekam_home_is_docker(self, tmp_path, monkeypatch):
        """When TCHUEKAM_HOME points outside ~/.tchuekam (Docker), returns TCHUEKAM_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(docker_home))
        assert get_default_tchuekam_root() == docker_home

    def test_tchuekam_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any TCHUEKAM_HOME outside ~/.tchuekam is treated as the root."""
        custom = tmp_path / "my-tchuekam-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(custom))
        assert get_default_tchuekam_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (TCHUEKAM_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(profile))
        assert get_default_tchuekam_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(tchuekam_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(tchuekam_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".tchuekam"
