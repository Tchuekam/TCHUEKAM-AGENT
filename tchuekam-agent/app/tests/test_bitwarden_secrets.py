...
binary download are exercised manually by `tchuekam secrets bitwarden
...
def tchuekam_home(tmp_path, monkeypatch):
    """Point TchuEkaM at an isolated home directory."""
    home = tmp_path / ".tchuekam"
    home.mkdir()
    monkeypatch.setenv("TCHUEKAM_HOME", str(home))
    # Some modules cache get_tchuekam_home; clear if needed.
    import tchuekam_constants
    if hasattr(tchuekam_constants, "_TCHUEKAM_HOME_CACHE"):
        tchuekam_constants._TCHUEKAM_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(tchuekam_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(tchuekam_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(tchuekam_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from tchuekam_cli.env_loader import _apply_external_secret_sources
