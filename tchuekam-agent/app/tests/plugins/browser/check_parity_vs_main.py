...
Run from the PR worktree:
    ...
    cd ~/.tchuekam/tchuekam-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.tchuekam/tchuekam-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.tchuekam/tchuekam-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a tchuekam-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a tchuekam-agent checkout"
)
...
# Isolated TCHUEKAM_HOME for the config write.
