"""Tests for tchuekam_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every TchuEkaM entry point
(tchuekam, tchuekam-agent, tchuekam-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every TchuEkaM entry point imports tchuekam_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported tchuekam_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("tchuekam_bootstrap", None)
    import tchuekam_bootstrap  # noqa: WPS433
    return tchuekam_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the TchuEkaM CLI
...
class TestEntryPointsImportBootstrap:
    """Every TchuEkaM entry point must import tchuekam_bootstrap as its
    ...
    # Entry points that invoke TchuEkaM as a process.  Each one must
    # import tchuekam_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "tchuekam_cli/main.py",   # tchuekam CLI (console_script)
        "run_agent.py",          # tchuekam-agent (console_script)
        "acp_adapter/entry.py",  # tchuekam-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import tchuekam_bootstrap' and that
        ...
        half-finished ``tchuekam update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``tchuekam_bootstrap`` as a top-level module) leaves tchuekam
        ...
        """
        # Resolve relative to the tchuekam-agent repo root.  Tests live
        # at tests/test_tchuekam_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # tchuekam start even when tchuekam_bootstrap hasn't been
        ...
        assert first_import_name == "tchuekam_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'tchuekam_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import tchuekam_bootstrap' line to be the first import."
        )
