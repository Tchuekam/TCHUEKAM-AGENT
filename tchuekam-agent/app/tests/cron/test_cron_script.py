...
- Path resolution (absolute, relative to TCHUEKAM_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp TCHUEKAM_HOME."""
    tchuekam_home = tmp_path / ".tchuekam"
    tchuekam_home.mkdir()
    (tchuekam_home / "cron").mkdir()
    (tchuekam_home / "cron" / "output").mkdir()
    (tchuekam_home / "scripts").mkdir()
    monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "TCHUEKAM_DIR", tchuekam_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", tchuekam_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", tchuekam_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", tchuekam_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.tchuekam/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("TCHUEKAM_SESSION_PLATFORM") is None
        assert os.environ.get("TCHUEKAM_SESSION_CHAT_ID") is None
        assert os.environ.get("TCHUEKAM_SESSION_CHAT_NAME") is None
