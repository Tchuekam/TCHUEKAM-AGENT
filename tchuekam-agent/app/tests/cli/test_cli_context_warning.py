...
def _isolate(tmp_path, monkeypatch):
    """Isolate TCHUEKAM_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal TchuEkaMCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = TchuEkaMCLI.__new__(TchuEkaMCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["tchuekam-core"]
        obj.compact = False
