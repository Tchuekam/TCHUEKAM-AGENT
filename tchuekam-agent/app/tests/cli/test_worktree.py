...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("tchuekam/tchuekam-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned tchuekam/* and pr-* branches."""

    def test_prunes_orphaned_tchuekam_branch(self, git_repo):
        """tchuekam/tchuekam-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "tchuekam/tchuekam-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "tchuekam/tchuekam-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "tchuekam/tchuekam-deadbeef" in result.stdout
        ...
        assert "tchuekam/tchuekam-deadbeef" in orphaned
        ...
        assert "tchuekam/tchuekam-deadbeef" not in result.stdout
