...
See: https://github.com/NousResearch/tchuekam-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for tchuekam_constants.get_subprocess_home()."""

    def test_returns_none_when_tchuekam_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        tchuekam_home = tmp_path / ".tchuekam"
        tchuekam_home.mkdir()
        profile_home = tchuekam_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))
        from tchuekam_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".tchuekam" / "profiles"
        ...
        monkeypatch.setenv("TCHUEKAM_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("TCHUEKAM_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from tchuekam_constants import (
            get_tchuekam_home,
            reset_tchuekam_home_override,
            set_tchuekam_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_tchuekam_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_tchuekam_home_override(profile)
        try:
            assert get_tchuekam_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_tchuekam_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_tchuekam_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(tchuekam_home / "home")
    ...
    def test_no_injection_when_tchuekam_home_unset(self, monkeypatch):
        monkeypatch.delenv("TCHUEKAM_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["TCHUEKAM_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from tchuekam_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from tchuekam_cli.profiles import create_profile
