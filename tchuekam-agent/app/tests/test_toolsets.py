...
class TestToolsetConsistency:
    ...
    def test_tchuekam_platforms_share_core_tools(self):
        """All tchuekam-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on tchuekam-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["tchuekam-cli", "tchuekam-telegram", "tchuekam-discord", "tchuekam-whatsapp", "tchuekam-slack", "tchuekam-signal", "tchuekam-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_tchuekam_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("tchuekam-whatsapp")

    def test_tchuekam_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("tchuekam-api-server")
