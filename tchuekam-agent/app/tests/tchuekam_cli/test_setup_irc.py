"""Tests for IRC gateway configuration via `tchuekam setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh TchuEkaM install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new TchuEkaM install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#tchuekam")
            monkeypatch.setenv("IRC_NICKNAME", "tchuekam-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `tchuekam setup gateway` experience with IRC present."""
