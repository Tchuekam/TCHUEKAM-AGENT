"""Tests for the TchuEkaM plugin system (tchuekam_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<tchuekam_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<tchuekam_home>/plugins/``; we derive
    ``<tchuekam_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <TCHUEKAM_HOME>/config.yaml.
        # Config is always read from TCHUEKAM_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        tchuekam_home_str = os.environ.get("TCHUEKAM_HOME")
        if tchuekam_home_str:
            tchuekam_home = Path(tchuekam_home_str)
        else:
            tchuekam_home = base.parent
        tchuekam_home.mkdir(parents=True, exist_ok=True)
        cfg_path = tchuekam_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.tchuekam/plugins/ are discovered."""
        plugins_dir = tmp_path / "tchuekam_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path / "tchuekam_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("TCHUEKAM_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".tchuekam" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        tchuekam_home = tmp_path / "tchuekam_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under tchuekam_plugins.<name>."""
        plugins_dir = tmp_path / "tchuekam_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path / "tchuekam_test"))

        # Clean up any prior namespace module
        sys.modules.pop("tchuekam_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "tchuekam_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="tchuekam_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "tchuekam_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import tchuekam_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("tchuekam_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        tchuekam_home = tmp_path / "tchuekam_test"
        plugins_dir = tchuekam_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "tchuekam_test" / "plugins"
        # Set TCHUEKAM_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("tchuekam_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("tchuekam_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("tchuekam_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("tchuekam_cli.plugins.PluginContext.dispatch_tool.__module__", "tchuekam_cli.plugins"):
...
class TestPluginDebugLogging:
    """TCHUEKAM_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("TCHUEKAM_PLUGINS_DEBUG", raising=False)
        from tchuekam_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With TCHUEKAM_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("TCHUEKAM_PLUGINS_DEBUG", "1")
        from tchuekam_cli import plugins as plugins_mod
