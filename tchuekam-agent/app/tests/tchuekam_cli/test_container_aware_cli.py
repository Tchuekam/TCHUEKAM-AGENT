...
def container_env(tmp_path, monkeypatch):
    """Set up a fake TCHUEKAM_HOME with .container-mode file."""
    tchuekam_home = tmp_path / ".tchuekam"
    tchuekam_home.mkdir()
    monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))
    monkeypatch.delenv("TCHUEKAM_DEV", raising=False)

    container_mode = tchuekam_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=tchuekam-agent\n"
        "exec_user=tchuekam\n"
        "tchuekam_bin=/data/current-package/bin/tchuekam\n"
    )
    return tchuekam_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("tchuekam_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "tchuekam-agent"
    assert info["exec_user"] == "tchuekam"
    assert info["tchuekam_bin"] == "/data/current-package/bin/tchuekam"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("tchuekam_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_tchuekam_dev(container_env, monkeypatch):
    """Returns None when TCHUEKAM_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("TCHUEKAM_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_tchuekam_dev_zero(container_env, monkeypatch):
    """TCHUEKAM_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("TCHUEKAM_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tchuekam_home = Path(tmpdir) / ".tchuekam"
        tchuekam_home.mkdir()
        (tchuekam_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("tchuekam_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_tchuekam_home": lambda: tchuekam_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TCHUEKAM_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=tchuekam-custom\n"
        "exec_user=myuser\n"
        "tchuekam_bin=/opt/tchuekam/bin/tchuekam\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "tchuekam-custom"
    assert info["exec_user"] == "myuser"
    assert info["tchuekam_bin"] == "/opt/tchuekam/bin/tchuekam"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from tchuekam_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "tchuekam"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "tchuekam-agent" in cmd
    assert "/data/current-package/bin/tchuekam" in cmd
    assert "chat" in cmd
