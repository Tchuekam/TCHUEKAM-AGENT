"""Tests for the Kanban DB layer (tchuekam_cli.kanban_db)."""
...
from tchuekam_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated TCHUEKAM_HOME with an empty kanban DB."""
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("TCHUEKAM_HOME", str(home))
    monkeypatch.delenv("TCHUEKAM_KANBAN_DB", raising=False)
    monkeypatch.delenv("TCHUEKAM_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("TCHUEKAM_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import tchuekam_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from tchuekam_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real TchuEkaM profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="tchuekam-task", assignee="daily")
...
def test_scratch_workspace_created_under_tchuekam_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "TchuEkaM-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``TCHUEKAM_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("TCHUEKAM_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """TchuEkaM' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` TchuEkaM' DB/metadata/logs on task completion.
    """
...
# spawned with `tchuekam -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's TCHUEKAM_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's TCHUEKAM_HOME."""

    def _set_home(self, monkeypatch, tmp_path, tchuekam_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))
        monkeypatch.delenv("TCHUEKAM_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_tchuekam(
        self, tmp_path, monkeypatch
    ):
        # Standard install: TCHUEKAM_HOME == ~/.tchuekam, no profile active.
        default_home = tmp_path / ".tchuekam"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.tchuekam/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.tchuekam/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.tchuekam/kanban.db.
        default_home = tmp_path / ".tchuekam"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # TCHUEKAM_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `tchuekam -p coder`).
    ...
    def test_docker_custom_tchuekam_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: TCHUEKAM_HOME points outside ~/.tchuekam.
        # `get_default_tchuekam_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".tchuekam"`.
        custom_root = tmp_path / "opt" / "tchuekam"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: TCHUEKAM_HOME=/opt/tchuekam/profiles/coder;
        # `get_default_tchuekam_root()` walks up to /opt/tchuekam because
    ...
    def test_explicit_override_via_tchuekam_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: TCHUEKAM_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".tchuekam"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(profile_home))
        monkeypatch.setenv("TCHUEKAM_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(default_home))
        monkeypatch.setenv("TCHUEKAM_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # TCHUEKAM_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile TCHUEKAM_HOME and reads.
        monkeypatch.setenv("TCHUEKAM_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_tchuekam_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # TCHUEKAM_KANBAN_DB pins the file path directly and beats both
        # TCHUEKAM_KANBAN_HOME and the `get_default_tchuekam_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(default_home))
        monkeypatch.setenv("TCHUEKAM_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("TCHUEKAM_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows TCHUEKAM_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_tchuekam_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # TCHUEKAM_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".tchuekam"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("TCHUEKAM_HOME", str(default_home))
        monkeypatch.setenv("TCHUEKAM_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("TCHUEKAM_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows TCHUEKAM_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        # TCHUEKAM_KANBAN_HOME.
        default_home = tmp_path / ".tchuekam"
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TCHUEKAM_HOME", str(default_home))
        monkeypatch.setenv("TCHUEKAM_KANBAN_DB", "   ")
        monkeypatch.setenv("TCHUEKAM_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject TCHUEKAM_KANBAN_DB
        # and TCHUEKAM_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites TCHUEKAM_HOME.
        default_home = tmp_path / ".tchuekam"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["TCHUEKAM_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["TCHUEKAM_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["TCHUEKAM_KANBAN_TASK"] == "t_dispatch_env"
        assert env["TCHUEKAM_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see tchuekam_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    (see tchuekam-agent issue #22032).
    """
    ...
    with _patch("tchuekam_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="tchuekam_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'tchuekam kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_tchuekam_argv()
#
# Workers spawned by the dispatcher must use a `tchuekam` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["tchuekam", ...]`
...
def test_resolve_tchuekam_argv_prefers_path_shim(monkeypatch):
    """When `tchuekam` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import tchuekam_cli.kanban_db as kb

    monkeypatch.delenv("TCHUEKAM_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/tchuekam")
    argv = kb._resolve_tchuekam_argv()
    assert argv == ["/usr/local/bin/tchuekam"]


def test_resolve_tchuekam_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import tchuekam_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("TCHUEKAM_BIN", ".\\tchuekam.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_tchuekam_argv() == [os.path.abspath(".\\tchuekam.exe")]


def test_resolve_tchuekam_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "tchuekam.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("TCHUEKAM_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_tchuekam_argv() == [sys.executable, "-m", "tchuekam_cli.main"]


def test_resolve_tchuekam_argv_honors_tchuekam_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like TCHUEKAM_BIN lets service managers pin the executable."""
    import shutil
    import tchuekam_cli.kanban_db as kb

    shim = tmp_path / "bin" / "tchuekam"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("TCHUEKAM_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_tchuekam_argv() == [str(shim)]


def test_resolve_tchuekam_argv_tchuekam_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare TCHUEKAM_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import tchuekam_cli.kanban_db as kb

    cwd_tchuekam = tmp_path / "tchuekam"
    cwd_tchuekam.write_text("wrong\n", encoding="utf-8")
    cwd_tchuekam.chmod(cwd_tchuekam.stat().st_mode | stat.S_IXUSR)
    path_tchuekam = tmp_path / "bin" / "tchuekam"
    path_tchuekam.parent.mkdir()
    path_tchuekam.write_text("right\n", encoding="utf-8")
    path_tchuekam.chmod(path_tchuekam.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_tchuekam.parent))
    monkeypatch.setenv("TCHUEKAM_BIN", "tchuekam")

    assert kb._resolve_tchuekam_argv() == [str(path_tchuekam)]


def test_resolve_tchuekam_argv_tchuekam_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare TCHUEKAM_BIN does not accept current-directory shadow executables."""
    import sys
    import tchuekam_cli.kanban_db as kb

    (tmp_path / "tchuekam.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_tchuekam_argv_tchuekam_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved TCHUEKAM_BIN batch shim is not used as worker argv[0]."""
    import sys
    import tchuekam_cli.kanban_db as kb
...
def test_resolve_tchuekam_argv_tchuekam_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved TCHUEKAM_BIN command names do not delegate cwd search to Popen."""
    import sys
    import tchuekam_cli.kanban_db as kb
...
def test_resolve_tchuekam_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m tchuekam_cli.main`.

    Pins the correct module name (NOT `tchuekam` — there is no top-level
    `tchuekam` package). Regression for #23198: the original PR shipped
    `python -m tchuekam` which fails with `No module named tchuekam` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_tchuekam_argv()
    assert argv == [sys.executable, "-m", "tchuekam_cli.main"]


def test_resolve_tchuekam_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `tchuekam_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m tchuekam_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("TCHUEKAM_BIN", None)
    ...
    assert "TchuEkaM Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="tchuekam_cli.kanban_db"):
        with kb.connect() as conn:
