...
Before the fix, ``tchuekam whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``tchuekam gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    tchuekam = home / ".tchuekam"
    tchuekam.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam))
...
def _env_value(tchuekam_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("tchuekam_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``tchuekam whatsapp`` with an existing paired session and
