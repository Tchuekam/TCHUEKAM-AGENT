"""Unit tests for tchuekam_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf tchuekam-ok"])
        try:
            output = _read_until(bridge, b"tchuekam-ok")
            assert b"tchuekam-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$TCHUEKAM_PTY_TEST\""],
            env={**os.environ, "TCHUEKAM_PTY_TEST": "pty-env-works"},
        )
        try:
