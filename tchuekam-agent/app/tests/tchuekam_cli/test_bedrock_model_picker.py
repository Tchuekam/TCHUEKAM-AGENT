...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (TCHUEKAM_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from tchuekam_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via TCHUEKAM_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in TCHUEKAM_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from tchuekam_cli.providers import TCHUEKAM_OVERLAYS
        assert "bedrock" in TCHUEKAM_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from tchuekam_cli.providers import TCHUEKAM_OVERLAYS
        assert TCHUEKAM_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from tchuekam_cli.providers import TCHUEKAM_OVERLAYS
        assert TCHUEKAM_OVERLAYS["bedrock"].auth_type == "aws_sdk"
