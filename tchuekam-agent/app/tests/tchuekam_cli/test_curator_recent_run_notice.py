...
The notice prints the most recent curator run summary on `tchuekam update`,
...
session start) so users normally never see the rename map. `tchuekam update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(tchuekam_main)
