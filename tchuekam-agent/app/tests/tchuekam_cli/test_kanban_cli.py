"""Tests for the kanban CLI surface (tchuekam_cli.kanban)."""
...
from tchuekam_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`tchuekam kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from tchuekam_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("TCHUEKAM_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("TCHUEKAM_KANBAN_BOARD") == "beta"
