...
``tchuekam_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``tchuekam --help``, ``tchuekam version``,
``tchuekam logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``tchuekam --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from tchuekam_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["tchuekam", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["tchuekam", "-w", "--tui"], None),
        (["tchuekam", "version"], "version"),
        (["tchuekam", "--tui", "chat"], "chat"),
        (["tchuekam", "-w", "logs"], "logs"),
        (["tchuekam", "chat", "hello world"], "chat"),
        (["tchuekam", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["tchuekam", "-m", "gpt5", "chat"], "chat"),
        (["tchuekam", "--model", "gpt5", "chat", "hi"], "chat"),
        (["tchuekam", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["tchuekam", "-z", "hello world"], None),
        (["tchuekam", "-z", "hello", "chat"], "chat"),
        (["tchuekam", "--model=gpt5", "chat"], "chat"),     # inline form
        (["tchuekam", "--", "chat"], "chat"),               # -- terminator
        (["tchuekam", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["tchuekam", "some-plugin-cmd"], "some-plugin-cmd"),
        (["tchuekam", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["tchuekam"],                          # bare → chat
        ["tchuekam", "--help"],                # top-level help
        ["tchuekam", "-h"],
        ["tchuekam", "version"],               # known built-in
        ["tchuekam", "logs"],
        ["tchuekam", "gateway", "run"],
        ["tchuekam", "--tui"],
        ["tchuekam", "-w", "--tui"],
        ["tchuekam", "chat", "hi"],
        ["tchuekam", "help"],                  # accepted built-in-ish
        ["tchuekam", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["tchuekam", "meet", "join"],          # potential google_meet plugin
        ["tchuekam", "honcho", "status"],      # potential memory plugin
        ["tchuekam", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``tchuekam help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"tchuekam_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
