"""Tests for the top-level `./tchuekam` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./tchuekam` should use `tchuekam_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "tchuekam"
    called = []

    fake_main_module = types.ModuleType("tchuekam_cli.main")

    def fake_main():
        called.append("tchuekam_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "tchuekam_cli.main", fake_main_module)
    ...
    assert called == ["tchuekam_cli.main"]
