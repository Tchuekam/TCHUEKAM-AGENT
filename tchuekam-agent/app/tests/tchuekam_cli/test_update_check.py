"""Tests for the update check mechanism in tchuekam_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "tchuekam-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("tchuekam_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path))
    with patch("tchuekam_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import tchuekam_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "tchuekam_cli" / "banner.py"
    ...
    monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path))
    with patch("tchuekam_cli.banner.subprocess.run") as mock_run:
        with patch("tchuekam_cli.banner.check_via_pypi", return_value=0):
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when TCHUEKAM_HOME has no git repo."""
    import tchuekam_cli.banner as banner
    ...
    # Point TCHUEKAM_HOME at a temp dir with no tchuekam-agent/.git
    monkeypatch.setenv("TCHUEKAM_HOME", str(tmp_path))
    with patch("tchuekam_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from tchuekam_cli.main import _invalidate_update_cache

    # Build a fake ~/.tchuekam with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"TCHUEKAM_HOME": str(default_home)}):
