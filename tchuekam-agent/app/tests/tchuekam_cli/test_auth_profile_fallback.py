...
When ``TCHUEKAM_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.tchuekam/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.tchuekam            (has its own auth.json fixture)
    * Profile     -> tmp_path/.tchuekam/profiles/coder   (active, TCHUEKAM_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (TCHUEKAM_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.tchuekam as the "real home" — which is NOT equal
    # to the TCHUEKAM_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    tchuekam_home = tmp_path / "classic"
    tchuekam_home.mkdir()
    monkeypatch.setenv("TCHUEKAM_HOME", str(tchuekam_home))

    _write(tchuekam_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from tchuekam_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: TCHUEKAM_HOME is set to a custom path that is NOT under
    # ~/.tchuekam/profiles/ — get_default_tchuekam_root() returns TCHUEKAM_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from tchuekam_cli.auth import read_credential_pool, write_credential_pool
