"""
Tests for timezone support (tchuekam_time module + integration points).
...
import tchuekam_time


def _reset_tchuekam_time_cache():
    """Reset the tchuekam_time module cache (replacement for removed reset_cache)."""
    tchuekam_time._cached_tz = None
    tchuekam_time._cached_tz_name = None
    tchuekam_time._cache_resolved = False


# =========================================================================
# tchuekam_time.now() — core helper
# =========================================================================

class TestTchuEkaMTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_tchuekam_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["TCHUEKAM_TIMEZONE"] = "Asia/Kolkata"
        result = tchuekam_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["TCHUEKAM_TIMEZONE"] = "UTC"
        result = tchuekam_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["TCHUEKAM_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="tchuekam_time"):
            result = tchuekam_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["TCHUEKAM_TIMEZONE"] = "UTC"
        _reset_tchuekam_time_cache()
        r1 = tchuekam_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["TCHUEKAM_TIMEZONE"] = "Asia/Kolkata"
        _reset_tchuekam_time_cache()
        r2 = tchuekam_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["TCHUEKAM_TIMEZONE"] = "Europe/London"
        tz = tchuekam_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["TCHUEKAM_TIMEZONE"] = "Not/A/Timezone"
        tz = tchuekam_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When TCHUEKAM_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) TCHUEKAM_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("TCHUEKAM_TIMEZONE=" + os.environ.get("TCHUEKAM_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "TCHUEKAM_TIMEZONE=NOT_SET" in result["output"], (
            "TCHUEKAM_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When TCHUEKAM_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("TCHUEKAM_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=tchuekam_tz) which shifted
        absolute time when system-local tz != TchuEkaM tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_tchuekam_tz(self):
        """Already-aware datetimes should be normalized to TchuEkaM tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in TchuEkaM tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of TchuEkaM tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), TchuEkaM is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when TchuEkaM tz
        ...
        # Use a TchuEkaM timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _tchuekam_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["TCHUEKAM_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_tchuekam_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of TchuEkaM timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["TCHUEKAM_TIMEZONE"] = "US/Eastern"
        _reset_tchuekam_time_cache()
