...
def server():
    with patch.dict(
        "sys.modules",
        {
            "tchuekam_constants": MagicMock(
                get_tchuekam_home=MagicMock(return_value="/tmp/tchuekam_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'tchuekam-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'tchuekam-release' patched"
    }
