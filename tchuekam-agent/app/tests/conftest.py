"""Shared fixtures for the tchuekam-agent test suite.
...
2. **Isolated TCHUEKAM_HOME.** TCHUEKAM_HOME points to a per-test tempdir so
   code reading ``~/.tchuekam/*`` via ``get_tchuekam_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".tchuekam"`` instead of the canonical
   ``get_tchuekam_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No TCHUEKAM_SESSION_* inheritance** — the agent's current gateway
...
# TCHUEKAM_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_TCHUEKAM_BEHAVIORAL_VARS = frozenset({
    "TCHUEKAM_YOLO_MODE",
    "TCHUEKAM_INTERACTIVE",
    "TCHUEKAM_QUIET",
    "TCHUEKAM_TOOL_PROGRESS",
    "TCHUEKAM_TOOL_PROGRESS_MODE",
    "TCHUEKAM_MAX_ITERATIONS",
    "TCHUEKAM_SESSION_PLATFORM",
    "TCHUEKAM_SESSION_CHAT_ID",
    "TCHUEKAM_SESSION_CHAT_NAME",
    "TCHUEKAM_SESSION_THREAD_ID",
    "TCHUEKAM_SESSION_SOURCE",
    "TCHUEKAM_SESSION_KEY",
    "TCHUEKAM_GATEWAY_SESSION",
    "TCHUEKAM_PLATFORM",
    "TCHUEKAM_MODEL",
    "TCHUEKAM_INFERENCE_MODEL",
    "TCHUEKAM_INFERENCE_PROVIDER",
    "TCHUEKAM_TUI_PROVIDER",
    "TCHUEKAM_MANAGED",
    "TCHUEKAM_DEV",
    "TCHUEKAM_CONTAINER",
    "TCHUEKAM_EPHEMERAL_SYSTEM_PROMPT",
    "TCHUEKAM_TIMEZONE",
    "TCHUEKAM_REDACT_SECRETS",
    "TCHUEKAM_BACKGROUND_NOTIFICATIONS",
    "TCHUEKAM_EXEC_ASK",
    "TCHUEKAM_HOME_MODE",
    "TCHUEKAM_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.tchuekam/kanban.db instead of the per-test TCHUEKAM_HOME.
    "TCHUEKAM_KANBAN_DB",
    "TCHUEKAM_KANBAN_BOARD",
    "TCHUEKAM_KANBAN_HOME",
    "TCHUEKAM_KANBAN_WORKSPACES_ROOT",
    "TCHUEKAM_KANBAN_LOGS_ROOT",
    "TCHUEKAM_KANBAN_TASK",
    "TCHUEKAM_KANBAN_WORKSPACE",
    "TCHUEKAM_KANBAN_RUN_ID",
    "TCHUEKAM_KANBAN_CLAIM_LOCK",
    "TCHUEKAM_KANBAN_DISPATCH_IN_GATEWAY",
    "TCHUEKAM_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and TCHUEKAM_HOME to per-test tempdirs so code that
    reads ``~/.tchuekam/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral TCHUEKAM_* vars that could change test semantics.
    for name in _TCHUEKAM_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect TCHUEKAM_HOME to a per-test tempdir. Code that reads
    #    ``~/.tchuekam/*`` via ``get_tchuekam_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.tchuekam/*`` via
    #    ``Path.home() / ".tchuekam"`` instead of ``get_tchuekam_home()``
    #    is a bug to fix at the callsite.
    fake_tchuekam_home = tmp_path / "tchuekam_test"
    fake_tchuekam_home.mkdir()
    (fake_tchuekam_home / "sessions").mkdir()
    (fake_tchuekam_home / "cron").mkdir()
    (fake_tchuekam_home / "memories").mkdir()
    (fake_tchuekam_home / "skills").mkdir()
    monkeypatch.setenv("TCHUEKAM_HOME", str(fake_tchuekam_home))
    ...
    #    ~/.tchuekam/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import tchuekam_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_tchuekam_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal tchuekam config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``tchuekam-gateway`` process
...
# correlated with a ``tests/tchuekam_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> tchuekam-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart tchuekam-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting tchuekam/python patterns are also blocked.
    """
    ...
    def _matches_tchuekam_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _TCHUEKAM_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_tchuekam_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch tchuekam-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m tchuekam_cli.main".
                if (
                    "tchuekam" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live tchuekam-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting tchuekam/python could hit the live gateway. "
