...
app.message(async ({ message, say }) => {
  ...
  if (!("subtype" in message) || message.subtype === undefined) {
    ...
    if (message.channel_type === "im") {
      await say(`TchuEkaM received your signal: "${(message as any).text}". System is operational.`);
    } 
    // If it's in a channel, only respond if it contains 'hello' (to avoid spam)
      ...
      await say(`TchuEkaM active in <#${message.channel}>. Signal received from <@${message.user}>.`);
    }
  }
});
...
app.event("app_mention", async ({ event, say }) => {
  await say(`TchuEkaM online. How can I assist, <@${event.user}>?`);
});

(async () => {
  try {
    await app.start();
    console.log("⚡️ TchuEkaM system online (Socket Mode). Listening for signals.");
  } catch (error) {
